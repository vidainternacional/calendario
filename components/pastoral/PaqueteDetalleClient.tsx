...
export default function PaqueteDetalleClient({ paquete, bosquejo, coleccion, recursos, pdfPresentacion, bosquejos, colecciones, biblioteca }: {
  ...
  return <>
    ...
    {modo === 'editar' ? <form action={guardar} className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
      <div className="space-y-5">
        <section className="rounded-[22px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="grid gap-4 sm:grid-cols-2">
          <label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-bold text-slate-700">Título</span><input name="titulo" value={titulo} onChange={(e) => setTitulo(e.target.value)} required maxLength={140} className="min-h-12 w-full rounded-xl border border-slate-200 px-3" /></label>
          <label><span className="mb-1.5 block text-xs font-bold text-slate-700">Estado</span><select name="estado" value={estado} onChange={(e) => setEstado(e.target.value as Paquete['estado'])} className="min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3"><option value="borrador">Borrador</option><option value="listo">Listo</option><option value="compartido">Compartido</option></select></label>
          <label><span className="mb-1.5 block text-xs font-bold text-slate-700">Bosquejo</span><select name="bosquejo_id" defaultValue={paquete.bosquejo_id ?? ''} className="min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3"><option value="">Sin bosquejo</option>{bosquejos.map((item) => <option key={item.id} value={item.id}>{item.titulo}</option>)}</select></label>
          <label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-bold text-slate-700">Colección de versículos</span><select name="coleccion_id" defaultValue={paquete.coleccion_id ?? ''} className="min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3"><option value="">Sin colección</option>{colecciones.map((item) => <option key={item.id} value={item.id}>{item.titulo}</option>)}</select></label>
          <label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-bold text-slate-700">Introducción para la congregación</span><textarea name="descripcion_publica" value={descripcion} onChange={(e) => setDescripcion(e.target.value)} rows={4} maxLength={2000} className="w-full rounded-xl border border-slate-200 p-3 leading-7" /></label>
          <label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-bold text-slate-700">Aplicación y preguntas</span><textarea name="instrucciones" value={instrucciones} onChange={(e) => setInstrucciones(e.target.value)} rows={5} maxLength={3000} className="w-full rounded-xl border border-slate-200 p-3 leading-7" /></label>
        </div></section>
        ...
        <section className="rounded-[22px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-center justify-between gap-3"><div><h2 className="font-bold text-slate-900">Presentación para pantalla</h2><p className="mt-1 text-xs text-slate-500">Una diapositiva por vez, con texto breve e imagen opcional.</p></div><button type="button" onClick={() => setDiapositivas((d) => [...d, { titulo: '', contenido: '', recurso_id: null }].slice(0, 30))} className="flex min-h-11 items-center gap-2 rounded-xl bg-indigo-50 px-3 text-xs font-bold text-indigo-700"><Plus className="h-4 w-4" />Diapositiva</button></div>
          <div className="mt-4 space-y-4">{diapositivas.map((item, index) => <article key={index} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-center justify-between"><span className="text-xs font-bold uppercase text-indigo-600">Diapositiva {index + 1}</span>{diapositivas.length > 1 && <button type="button" onClick={() => setDiapositivas((d) => d.filter((_, i) => i !== index))} className="h-9 w-9 rounded-full bg-rose-50 text-rose-500"><Trash2 className="mx-auto h-4 w-4" /></button>}</div><input name="diapositiva_titulo" value={item.titulo} onChange={(e) => actualizarDiapositiva(index, 'titulo', e.target.value)} placeholder="Título" className="mt-3 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3 font-semibold" /><textarea name="diapositiva_contenido" value={item.contenido} onChange={(e) => actualizarDiapositiva(index, 'contenido', e.target.value)} rows={4} placeholder="Texto breve para proyectar" className="mt-3 w-full rounded-xl border border-slate-200 bg-white p-3 leading-7" /><select name="diapositiva_recurso_id" value={item.recurso_id ?? ''} onChange={(e) => actualizarDiapositiva(index, 'recurso_id', e.target.value || null)} className="mt-3 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3"><option value="">Sin imagen</option>{imagenes.map((imagen) => <option key={imagen.id} value={imagen.id}>{imagen.titulo}</option>)}</select></article>)}</div>
          <label className="mt-5 block"><span className="mb-1.5 block text-xs font-bold text-slate-700">PDF de presentación opcional</span><select name="presentacion_pdf_recurso_id" defaultValue={paquete.presentacion_pdf_recurso_id ?? ''} className="min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3"><option value="">Sin PDF</option>{pdfs.map((pdf) => <option key={pdf.id} value={pdf.id}>{pdf.titulo}</option>)}</select><span className="mt-1.5 block text-xs text-slate-500">Suba primero el PDF a Biblioteca Pastoral y selecciónelo aquí.</span></label>
        </section>
