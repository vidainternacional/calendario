    ...
    <>
      ...
      <article className="pastoral-print mt-5 rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        ...
        {coleccion.versiculos.length === 0 ? (
          <div className="py-12 text-center"><p className="font-semibold text-slate-800">Esta guía todavía no tiene versículos</p><p className="mt-1 text-sm text-slate-500">Usa la concordancia o selecciona una referencia para comenzar.</p></div>
        ) : (
              ...
              return (
                <section key={item.id} className="break-inside-avoid rounded-2xl border border-slate-100 bg-slate-50 p-4 sm:p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div><p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Pasaje {index + 1}</p><h2 className="mt-1 font-bold text-indigo-700">{referencia}</h2><p className="mt-0.5 text-[11px] text-slate-400">{item.traduccion}</p></div>
                    <button type="button" onClick={() => eliminar(item.id, referencia)} disabled={isPending} className="print:hidden flex h-10 w-10 items-center justify-center rounded-full bg-rose-50 text-rose-500" aria-label={`Eliminar ${referencia}`}><Trash2 className="h-4 w-4" /></button>
                  </div>
      ...
      {versiculoAbierto && (
        <div className="modal-overlay-safe bg-black/55" onClick={(event) => { if (event.target === event.currentTarget) setVersiculoAbierto(false) }}>
          <section className="modal-panel-safe rounded-3xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="agregar-versiculo-title">
            <header className="flex items-center justify-between border-b border-slate-100 px-5 py-4"><div><h2 id="agregar-versiculo-title" className="font-bold text-slate-900">Buscar en la Biblia</h2><p className="mt-0.5 text-xs text-slate-500">Busca concordancias o selecciona una referencia.</p></div><button type="button" onClick={() => setVersiculoAbierto(false)} className="flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 text-slate-600" aria-label="Cerrar"><X className="h-5 w-5" /></button></header>
            <div className="modal-body-safe p-5"><PastoralVerseFinder coleccionId={coleccion.id} onSaved={() => setVersiculoAbierto(false)} /></div>
          </section>
        </div>
      )}
