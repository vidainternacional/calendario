...
type GrupoTexto = 'fuente' | 'rol' | 'formato' | 'tamano' | 'alineacion' | 'listas'
...
export default function PastoralVisualWorkspaceV4({ paquete, biblioteca }: { paquete: Paquete; coleccion: unknown; biblioteca: RecursoPastoral[] }) {
  ...
  const [grupoTextoAbierto, setGrupoTextoAbierto] = useState<GrupoTexto | null>(null)
  ...
  const panelContenido = pagina && panel ? <>
    ...
    {panel === 'fondos' && <div className="pastoral-panel-content pastoral-start-panel">
    ...
    {panel === 'texto' && <div className="pastoral-panel-content grid w-full content-start gap-2.5">
      <div className="grid w-full grid-cols-[minmax(0,1fr)_minmax(0,1fr)_44px] gap-2">
        <button type="button" disabled={!textoSeleccionado} onClick={() => alternarGrupoTexto('fuente')} className="flex min-h-10 min-w-0 items-center justify-between gap-1 rounded-full border border-slate-200 bg-white px-3 text-[11px] font-bold text-slate-700 disabled:opacity-35" aria-expanded={grupoTextoAbierto === 'fuente'} aria-label={`Fuente: ${fuenteTextoActual}`}><span className="truncate" style={{ fontFamily: textoSeleccionado?.fuente ?? 'Inter' }}>{fuenteTextoActual}</span><span className="shrink-0 text-slate-400">⌄</span></button>
        <button type="button" onClick={() => alternarGrupoTexto('rol')} className="flex min-h-10 min-w-0 items-center justify-between gap-1 rounded-full border border-slate-200 bg-white px-3 text-[11px] font-bold text-slate-700" aria-expanded={grupoTextoAbierto === 'rol'} aria-label={`Tipo de texto: ${etiquetaRolTexto}`}><span className="truncate">{etiquetaRolTexto}</span><span className="shrink-0 text-slate-400">⌄</span></button>
        <button type="button" disabled={!textoSeleccionado} onClick={() => { if (textoSeleccionado) eliminarElemento(textoSeleccionado.id); setGrupoTextoAbierto(null) }} className="grid min-h-10 w-11 place-items-center rounded-full border border-rose-200 bg-white text-rose-600 disabled:opacity-30" aria-label="Eliminar texto seleccionado" title="Eliminar texto"><Trash2 className="h-[18px] w-[18px]" /></button>
      </div>

      <div className="flex w-full gap-2 overflow-x-auto pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <button type="button" disabled={!textoSeleccionado} onClick={() => alternarGrupoTexto('formato')} className={`min-h-10 shrink-0 rounded-full border px-3 text-[11px] font-bold ${grupoTextoAbierto === 'formato' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-700'} disabled:opacity-35`} aria-expanded={grupoTextoAbierto === 'formato'}>Formato</button>
        <button type="button" disabled={!textoSeleccionado} onClick={() => alternarGrupoTexto('tamano')} className={`min-h-10 shrink-0 rounded-full border px-3 text-[11px] font-bold ${grupoTextoAbierto === 'tamano' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-700'} disabled:opacity-35`} aria-expanded={grupoTextoAbierto === 'tamano'}>Tamaño</button>
        <button type="button" disabled={!textoSeleccionado} onClick={() => alternarGrupoTexto('alineacion')} className={`min-h-10 shrink-0 rounded-full border px-3 text-[11px] font-bold ${grupoTextoAbierto === 'alineacion' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-700'} disabled:opacity-35`} aria-expanded={grupoTextoAbierto === 'alineacion'}>Alinear</button>
        <button type="button" disabled={!textoSeleccionado} onClick={() => alternarGrupoTexto('listas')} className={`min-h-10 shrink-0 rounded-full border px-3 text-[11px] font-bold ${grupoTextoAbierto === 'listas' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-700'} disabled:opacity-35`} aria-expanded={grupoTextoAbierto === 'listas'}>Listas</button>
      </div>

      {grupoTextoAbierto && <div className="max-h-44 w-full overflow-y-auto border-t border-slate-200 pt-2.5">
        {grupoTextoAbierto === 'fuente' && <div className="grid grid-cols-2 gap-2" aria-label="Fuentes disponibles">{FUENTES_PASTORALES.map((fuente) => <button key={fuente} type="button" disabled={!textoSeleccionado} onClick={() => { if (textoSeleccionado) actualizarElemento(textoSeleccionado.id, { fuente }); setGrupoTextoAbierto(null) }} className={`min-h-11 min-w-0 rounded-full border px-3 text-xs font-bold ${textoSeleccionado?.fuente === fuente ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600'}`} style={{ fontFamily: fuente }}><span className="block truncate">{fuente}</span></button>)}</div>}

        {grupoTextoAbierto === 'rol' && <div className="flex w-full gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Opciones de tipo de texto"><button type="button" onClick={() => { agregarTexto('libre'); setGrupoTextoAbierto(null) }} className={`min-h-11 shrink-0 rounded-full border px-4 text-xs font-bold ${textoSeleccionado?.rol === 'libre' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600'}`}>Caja</button>{ESTILOS_TEXTO.filter((item) => item.id !== 'libre').map((estilo) => <button key={estilo.id} type="button" onClick={() => { aplicarRolTexto(estilo.id); setGrupoTextoAbierto(null) }} className={`min-h-11 shrink-0 rounded-full border px-4 text-xs font-bold ${textoSeleccionado?.rol === estilo.id ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600'}`}>{estilo.label}</button>)}</div>}

        {grupoTextoAbierto === 'formato' && <div className="grid gap-3">
          <div className="flex flex-wrap gap-2" role="group" aria-label="Formato de texto">
          ...
          <div className="flex flex-wrap gap-2" role="group" aria-label="Color de texto">{COLORES_TEXTO.map((color) => <button key={color} type="button" disabled={!textoSeleccionado} onClick={() => textoSeleccionado && actualizarElemento(textoSeleccionado.id, { color })} className={`h-9 w-9 rounded-full border-2 ${textoSeleccionado?.color === color ? 'border-indigo-500' : 'border-slate-200'}`} style={{ backgroundColor: color }} aria-label={`Color de texto ${color}`} />)}</div>
        </div>}

        {grupoTextoAbierto === 'tamano' && <div className="grid gap-3">
          <div className="grid grid-cols-[auto_40px_minmax(54px,1fr)_40px_auto] items-center gap-2"><span className="text-[11px] font-bold text-slate-500">Tamaño</span><button type="button" disabled={!textoSeleccionado} onClick={() => ajustarTamano(-1)} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-base font-bold text-slate-700" aria-label="Reducir tamaño de letra">−</button><input aria-label="Tamaño de letra en puntos" type="number" min="8" max="160" disabled={!textoSeleccionado} value={Math.round(textoSeleccionado?.tamano_fuente ?? 24)} onChange={(e) => textoSeleccionado && actualizarElemento(textoSeleccionado.id, { tamano_fuente: Math.min(160, Math.max(8, Number(e.target.value) || 8)) })} className="h-10 min-w-0 rounded-full border border-slate-200 bg-white px-2 text-center text-xs font-bold text-slate-700" /><button type="button" disabled={!textoSeleccionado} onClick={() => ajustarTamano(1)} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-base font-bold text-slate-700" aria-label="Aumentar tamaño de letra">+</button><span className="text-[11px] font-bold text-slate-400">pt</span></div>
          <div className="grid grid-cols-[auto_40px_minmax(54px,1fr)_40px] items-center gap-2"><span className="text-[11px] font-bold text-slate-500">Línea</span><button type="button" disabled={!textoSeleccionado} onClick={() => ajustarLinea(-0.05)} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-base font-bold text-slate-700" aria-label="Reducir interlineado">−</button><input aria-label="Interlineado" type="number" min="0.9" max="2" step="0.05" disabled={!textoSeleccionado} value={textoSeleccionado?.interlineado ?? 1.25} onChange={(e) => textoSeleccionado && actualizarElemento(textoSeleccionado.id, { interlineado: Math.min(2, Math.max(.9, Number(e.target.value) || 1.25)) })} className="h-10 min-w-0 rounded-full border border-slate-200 bg-white px-2 text-center text-xs font-bold text-slate-700" /><button type="button" disabled={!textoSeleccionado} onClick={() => ajustarLinea(0.05)} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-base font-bold text-slate-700" aria-label="Aumentar interlineado">+</button></div>
        </div>}

        {grupoTextoAbierto === 'alineacion' && <div className="flex gap-2" role="group" aria-label="Alineación"><button type="button" disabled={!textoSeleccionado} onClick={() => { alinear('izquierda'); setGrupoTextoAbierto(null) }} className={`grid h-11 w-11 place-items-center rounded-full border ${textoSeleccionado?.alineacion === 'izquierda' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-700'}`} aria-label="Alinear a la izquierda"><AlignLeft className="h-4 w-4" /></button><button type="button" disabled={!textoSeleccionado} onClick={() => { alinear('centro'); setGrupoTextoAbierto(null) }} className={`grid h-11 w-11 place-items-center rounded-full border ${textoSeleccionado?.alineacion === 'centro' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-700'}`} aria-label="Centrar"><AlignCenter className="h-4 w-4" /></button><button type="button" disabled={!textoSeleccionado} onClick={() => { alinear('derecha'); setGrupoTextoAbierto(null) }} className={`grid h-11 w-11 place-items-center rounded-full border ${textoSeleccionado?.alineacion === 'derecha' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-700'}`} aria-label="Alinear a la derecha"><AlignRight className="h-4 w-4" /></button></div>}

        {grupoTextoAbierto === 'listas' && <div className="flex gap-2" role="group" aria-label="Listas"><button type="button" disabled={!textoSeleccionado} onMouseDown={(e) => e.preventDefault()} onClick={() => { comandoParrafo('insertUnorderedList'); setGrupoTextoAbierto(null) }} className="flex min-h-11 items-center gap-2 rounded-full border border-slate-200 bg-white px-4 text-xs font-bold text-slate-700" aria-label="Lista con viñetas"><List className="h-4 w-4" />Viñetas</button><button type="button" disabled={!textoSeleccionado} onMouseDown={(e) => e.preventDefault()} onClick={() => { comandoParrafo('insertOrderedList'); setGrupoTextoAbierto(null) }} className="flex min-h-11 items-center gap-2 rounded-full border border-slate-200 bg-white px-4 text-xs font-bold text-slate-700" aria-label="Lista numerada"><ListOrdered className="h-4 w-4" />Numerada</button></div>}
      </div>}
    </div>}
  ...
  return <div className="pastoral-content-workspace pastoral-canva-workspace pastoral-editor-v3 pastoral-editor-v4 text-slate-900">
    ...
    {vista === 'contenido' && pagina && <section className="pastoral-editor-section pb-4 pt-2"><div className={grupoPrincipal ? 'pastoral-editor-shell has-panel' : "pastoral-editor-shell-closed grid h-full min-h-0 w-full grid-cols-1 grid-rows-[minmax(0,1fr)_70px] overflow-hidden lg:grid-rows-[minmax(560px,calc(100dvh-215px))_64px]"} style={grupoPrincipal ? undefined : { gridTemplateAreas: "'stage' 'dock'" }}>
      <div className="pastoral-tool-dock" aria-label="Herramientas del lienzo">{HERRAMIENTAS.map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => alternarGrupoPrincipal(id)} className={`pastoral-tool-button col-span-2 ${grupoPrincipal === id ? 'is-active' : ''}`} aria-pressed={grupoPrincipal === id} aria-expanded={grupoPrincipal === id} aria-label={label} title={label}><Icon /><small className="relative z-[1] text-[10px] font-bold">{label}</small></button>)}</div>
      ...
      {grupoPrincipal && <aside className={`pastoral-tool-panel ${clasePanel}`} aria-label={`Panel ${grupoPrincipal}`}><div className="pastoral-tool-panel-scroll"><div className="flex h-full min-h-0 flex-col"><div className="flex shrink-0 items-center gap-1 overflow-x-auto border-b border-slate-200 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label={`Opciones de ${grupoPrincipal}`}>{SUBMENUS[grupoPrincipal].map((item) => <button key={item.id} type="button" onClick={() => alternarSubpanel(item.id)} className={`min-h-10 shrink-0 rounded-full px-3 text-xs font-bold ${panel === item.id ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500'}`} aria-pressed={panel === item.id}>{item.label}</button>)}</div><div className={`min-h-0 flex-1 pt-2 ${panel === 'biblia' ? 'overflow-hidden' : 'overflow-y-auto'}`}>{panelContenido}</div></div></div></aside>}
    </div></section>}
