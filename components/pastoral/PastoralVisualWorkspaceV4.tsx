  ...
  const panelContenido = pagina && panel ? <>
    ...
    {panel === 'texto' && <div className="pastoral-panel-content grid h-full w-full content-start gap-3 overflow-y-auto pr-1">
      ...
      <section className="grid gap-2 border-b border-slate-200 pb-3">
        ...
        <div className="flex gap-2" role="group" aria-label="Alineación"><button type="button" disabled={!textoSeleccionado} onClick={() => alinear('izquierda')} className={`grid h-11 w-11 place-items-center rounded-full border ${textoSeleccionado?.alineacion === 'izquierda' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-700'}`} aria-label="Alinear a la izquierda"><AlignLeft className="h-4 w-4" /></button><button type="button" disabled={!textoSeleccionado} onClick={() => alinear('centro')} className={`grid h-11 w-11 place-items-center rounded-full border ${textoSeleccionado?.alineacion === 'centro' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-700'}`} aria-label="Centrar"><AlignCenter className="h-4 w-4" /></button><button type="button" disabled={!textoSeleccionado} onClick={() => alinear('derecha')} className={`grid h-11 w-11 place-items-center rounded-full border ${textoSeleccionado?.alineacion === 'derecha' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-700'}`} aria-label="Alinear a la derecha"><AlignRight className="h-4 w-4" /></button></div>
      </section>

      <section className="grid gap-2 pb-2">
        <div className="px-1 text-[11px] font-black text-slate-500">Listas</div>
        <div className="flex flex-wrap gap-2" role="group" aria-label="Listas"><button type="button" disabled={!textoSeleccionado} onMouseDown={(e) => e.preventDefault()} onClick={() => comandoParrafo('insertUnorderedList')} className="flex min-h-11 items-center gap-2 rounded-full border border-slate-200 bg-white px-4 text-xs font-bold text-slate-700" aria-label="Lista con viñetas"><List className="h-4 w-4" />Viñetas</button><button type="button" disabled={!textoSeleccionado} onMouseDown={(e) => e.preventDefault()} onClick={() => comandoParrafo('insertOrderedList')} className="flex min-h-11 items-center gap-2 rounded-full border border-slate-200 bg-white px-4 text-xs font-bold text-slate-700" aria-label="Lista numerada"><ListOrdered className="h-4 w-4" />Numerada</button></div>
      </section>
    </div>}
