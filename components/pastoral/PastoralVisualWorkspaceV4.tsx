...
type TipoDegradadoPersonalizado = 'lineal' | 'radial' | 'conico'
...
export default function PastoralVisualWorkspaceV4({ paquete, biblioteca }: { paquete: Paquete; coleccion: unknown; biblioteca: RecursoPastoral[] }) {
  ...
  const [orientacionPresentacion, setOrientacionPresentacion] = useState<'horizontal' | 'vertical'>('horizontal')
  ...
  const [selectorDegradadosAbierto, setSelectorDegradadosAbierto] = useState(false)
  ...
  const aplicarFondoVisual = (paleta: { fondo: string }) => aplicarFondoSeleccionado(paleta.fondo)
...
// eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const panelContenido = pagina && panel ? <>
    ...
    {panel === 'fondos' && <div className="pastoral-panel-content grid gap-3 pb-3 pt-1">
      ...
      {selectorFondoAbierto && <section id="pastoral-selector-fondo-libre" className="grid gap-3 rounded-2xl border border-slate-200 bg-white/80 px-3 py-3">
        ...
        <div className="grid gap-2">
          ...
          <div className="flex touch-pan-x gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Tipos de textura personalizados">
      ...
      {selectorDegradadosAbierto && <section id="pastoral-selector-degradados" className="grid gap-3 rounded-2xl border border-slate-200 bg-white/80 px-3 py-3">
      ...
      {selectorImagenesAbierto && <section id="pastoral-selector-imagenes" className="grid gap-3 rounded-2xl border border-slate-200 bg-white/80 px-3 py-3">
        ...
        {recursosFiltrados.length ? <div className="grid grid-cols-3 gap-2">{recursosFiltrados.map((recurso) => <article key={recurso.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          ...
          <div className="grid grid-cols-2 border-t border-slate-100">
            <button type="button" onClick={() => usarRecursoComoImagen(recurso)} className={`min-h-9 border-r border-slate-100 px-1 text-[10px] font-black ${elementoSeleccionado?.tipo === 'imagen' && elementoSeleccionado.recurso_id === recurso.id && !elementoSeleccionado.es_capa_fondo ? 'text-indigo-600' : 'text-slate-600'}`} aria-pressed={Boolean(elementoSeleccionado?.tipo === 'imagen' && elementoSeleccionado.recurso_id === recurso.id && !elementoSeleccionado.es_capa_fondo)} aria-label={`Usar ${recurso.titulo} como imagen`}>Imagen</button>
            <button type="button" onClick={() => usarRecursoComoFondo(recurso)} className={`min-h-9 px-1 text-[10px] font-black ${((elementoSeleccionado?.tipo === 'imagen' && elementoSeleccionado.recurso_id === recurso.id && elementoSeleccionado.es_capa_fondo) || (pagina.fondo_modo === 'imagen' && (pagina.fondo_recurso_id ?? pagina.recurso_id) === recurso.id)) ? 'text-indigo-600' : 'text-slate-600'}`} aria-pressed={Boolean((elementoSeleccionado?.tipo === 'imagen' && elementoSeleccionado.recurso_id === recurso.id && elementoSeleccionado.es_capa_fondo) || (pagina.fondo_modo === 'imagen' && (pagina.fondo_recurso_id ?? pagina.recurso_id) === recurso.id))} aria-label={`Usar ${recurso.titulo} como fondo`}>Fondo</button>
          </div>
      ...
      <div className="pastoral-background-grid" aria-label="Galería de fondos">
        {[...paletasColoresFlat, ...paletasDegradados, ...degradadosGuardados, ...paletasTexturas].map((paleta) => <button key={paleta.id} type="button" onClick={() => aplicarFondoVisual(paleta)} className="pastoral-background-swatch" aria-label={`Aplicar fondo ${paleta.label}`} title={paleta.label}><span style={{ background: paleta.fondo }} /></button>)}
      </div>
    ...
    {panel === 'capas' && <div className="pastoral-panel-content grid gap-3">
      <div className="grid grid-cols-3 gap-3 border-b border-slate-200 px-1 pb-3" role="toolbar" aria-label="Herramientas rápidas de capas">
    ...
    {vista === 'presentacion' && pagina && <section className={modoPresentacion ? 'fixed inset-0 z-[170] flex h-[100dvh] w-[100dvw] items-center justify-center overflow-hidden bg-black overscroll-none' : 'relative pb-10 pt-5'}>
      {!modoPresentacion && <div className="mb-4 grid gap-3">
        ...
        <div className="inline-flex w-fit items-center rounded-full border border-slate-200 bg-white p-1" aria-label="Orientación de presentación">
          <button type="button" onClick={() => setOrientacionPresentacion('horizontal')} className={`min-h-10 rounded-full px-4 text-xs font-bold ${orientacionPresentacion === 'horizontal' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500'}`} aria-pressed={orientacionPresentacion === 'horizontal'}>Horizontal</button>
          <button type="button" onClick={() => setOrientacionPresentacion('vertical')} className={`min-h-10 rounded-full px-4 text-xs font-bold ${orientacionPresentacion === 'vertical' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500'}`} aria-pressed={orientacionPresentacion === 'vertical'}>Vertical</button>
        </div>
      </div>}
