  ...
  const panelContenido = pagina ? <>
    ...
    {panel === 'texto' && <div className="pastoral-panel-content grid w-full content-start gap-2.5">
      ...
      <div className="pastoral-inline-toolbar pastoral-text-tools-row">
        ...
        <span className="pastoral-text-divider" aria-hidden="true" />
        <div className="flex shrink-0 items-center gap-1" role="group" aria-label="Alineación">
        ...
        <span className="pastoral-text-divider" aria-hidden="true" />
        <div className="flex shrink-0 items-center gap-1" role="group" aria-label="Listas">
        ...
        <span className="pastoral-text-divider" aria-hidden="true" />
        <div className="flex shrink-0 items-center" role="group" aria-label="Color de texto"><div className="pastoral-color-strip">{COLORES_TEXTO.map((color) => <button key={color} type="button" disabled={!textoSeleccionado} onClick={() => textoSeleccionado && actualizarElemento(textoSeleccionado.id, { color })} className={textoSeleccionado?.color === color ? 'is-active' : ''} style={{ backgroundColor: color }} aria-label={`Color de texto ${color}`} />)}</div></div>
      </div>
    </div>}
