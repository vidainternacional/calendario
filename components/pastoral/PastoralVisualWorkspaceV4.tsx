...
export default function PastoralVisualWorkspaceV4({ paquete, biblioteca }: { paquete: Paquete; coleccion: unknown; biblioteca: RecursoPastoral[] }) {
    ...
    {panel === 'texto' && <div className="pastoral-panel-content grid w-full content-start gap-2.5">
      ...
      {grupoTextoAbierto && <div className="max-h-44 w-full overflow-y-auto border-t border-slate-200 pt-2.5">
        ...
        {grupoTextoAbierto === 'tamano' && <div className="grid gap-3">
          <div className="grid grid-cols-[auto_40px_minmax(54px,1fr)_40px_auto] items-center gap-2"><span className="text-[11px] font-bold text-slate-500">Tamaño</span><button type="button" disabled={!textoSeleccionado} onClick={() => ajustarTamano(-1)} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-base font-bold text-slate-700">−</button><input aria-label="Tamaño de letra en puntos" type="number" min="8" max="160" disabled={!textoSeleccionado} value={Math.round(textoSeleccionado?.tamano_fuente ?? 24)} onChange={(e) => textoSeleccionado && actualizarElemento(textoSeleccionado.id, { tamano_fuente: Math.min(160, Math.max(8, Number(e.target.value) || 8)) })} className="h-10 min-w-0 rounded-full border border-slate-200 bg-white px-2 text-center text-xs font-bold text-slate-700" /><button type="button" disabled={!textoSeleccionado} onClick={() => ajustarTamano(1)} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-base font-bold text-slate-700">+</button><span className="text-[11px] font-bold text-slate-400">pt</span></div>
          <div className="grid grid-cols-[auto_40px_minmax(54px,1fr)_40px] items-center gap-2"><span className="text-[11px] font-bold text-slate-500">Línea</span><button type="button" disabled={!textoSeleccionado} onClick={() => ajustarLinea(-0.05)} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-base font-bold text-slate-700">−</button><input aria-label="Interlineado" type="number" min="0.9" max="2" step="0.05" disabled={!textoSeleccionado} value={textoSeleccionado?.interlineado ?? 1.25} onChange={(e) => textoSeleccionado && actualizarElemento(textoSeleccionado.id, { interlineado: Math.min(2, Math.max(.9, Number(e.target.value) || 1.25)) })} className="h-10 min-w-0 rounded-full border border-slate-200 bg-white px-2 text-center text-xs font-bold text-slate-700" /><button type="button" disabled={!textoSeleccionado} onClick={() => ajustarLinea(0.05)} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-base font-bold text-slate-700">+</button></div>
        </div>}
  ...
  return <div className="pastoral-content-workspace pastoral-canva-workspace pastoral-editor-v3 pastoral-editor-v4 text-slate-900">
    ...
    {vista === 'contenido' && pagina && <section className="pastoral-editor-section pb-4 pt-2"><div className="pastoral-editor-shell has-panel">
      <div className="pastoral-tool-dock" aria-label="Herramientas del lienzo">{HERRAMIENTAS.map(({ id, label, icon: Icon }) => <button key={id} type="button" disabled={id === 'capas' && !(pagina.elementos?.length)} onClick={() => setPanel(id)} className={`pastoral-tool-button ${panel === id ? 'is-active' : ''}`} aria-pressed={panel === id} aria-label={label} title={label}><Icon /></button>)}</div>
