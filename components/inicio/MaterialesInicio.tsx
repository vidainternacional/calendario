    ...
    return (
      <section aria-label="Materiales para la iglesia">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {[0, 1].map((item) => <div key={item} className="h-36 animate-pulse rounded-2xl border border-amber-100 bg-white" />)}
        </div>
      </section>
    )
  }
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {materiales.map((material) => (
          <Link
            ...
            className="group flex min-h-36 flex-col rounded-2xl border border-amber-100 bg-gradient-to-br from-amber-50/70 to-white p-4 shadow-sm transition hover:border-amber-200 hover:shadow-md"
          >
            <div className="flex items-start justify-between gap-3">
              <span className="rounded-full border border-amber-100 bg-white/80 px-3 py-1 text-[10px] font-bold text-amber-800">
            ...
            <span className="mt-auto pt-3 text-xs font-bold text-amber-800">Abrir material</span>
          </Link>
        ))}
      </div>
    </section>
