      ...
      <div className="px-4 pt-4 sm:px-6">
        <div className="grid grid-cols-3 rounded-[18px] bg-white p-1 ring-1 ring-black/[0.05]">
          {([['solicitar', 'Necesito ayuda'], ['aportar', 'Quiero sembrar'], ['seguimiento', 'Seguimiento']] as Array<[Tab, string]>).map(([id, label]) => (
            <button key={id} onClick={() => { setTab(id); setMessage(null) }} className={`min-h-11 rounded-[14px] px-2 text-[12px] font-bold transition ${tab === id ? 'bg-[#5b3df5] text-white' : 'text-slate-700'}`}><span className="inline-flex items-center justify-center gap-1.5">{label}{id === 'seguimiento' ? <SolidarityUnreadBadge scope="all" /> : null}</span></button>
          ))}
        </div>
