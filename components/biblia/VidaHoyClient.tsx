    ...
    <main className="mx-auto min-h-screen max-w-2xl bg-white px-4 pb-[calc(7rem+env(safe-area-inset-bottom))] pt-[calc(1rem+env(safe-area-inset-top))] sm:px-6 sm:pt-7">
      <header className="relative flex items-start justify-between gap-4 pb-5">
        ...
        <details className="group relative shrink-0">
          <summary className="flex min-h-10 cursor-pointer list-none items-center gap-2 rounded-full border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 [&::-webkit-details-marker]:hidden">
            <Bell className="h-4 w-4 text-slate-500" />
            <span>{active ? String(hour).padStart(2, '0') + ':00' : 'Recordatorio'}</span>
            <ChevronDown className="h-3.5 w-3.5 text-slate-400 transition-transform group-open:rotate-180" />
          </summary>

          <div className="absolute right-0 top-12 z-20 w-[min(290px,calc(100vw-2rem))] rounded-2xl border border-slate-200 bg-white p-4 shadow-lg">
            <div>
              <p className="text-sm font-bold text-slate-900">Recordatorio diario</p>
              <p className="mt-1 text-xs leading-5 text-slate-500">Elige la hora en que quieres recibir el versículo.</p>
            </div>
            ...
            <div className="mt-4 flex items-center justify-between gap-3">
              ...
              <select value={hour} disabled={!active || isPending} onChange={e => saveReminder(true, Number(e.target.value))} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-800 disabled:opacity-50">
                {Array.from({ length: 18 }, (_, i) => i + 5).map(value => <option key={value} value={value}>{String(value).padStart(2, '0')}:00</option>)}
              </select>
            </div>
            <button type="button" onClick={() => saveReminder(!active)} disabled={isPending} className={`mt-4 min-h-10 rounded-xl px-4 text-sm font-bold transition ${active ? 'border border-slate-200 text-slate-700' : 'bg-[#C0392B] text-white'}`}>
              {isPending ? 'Guardando…' : active ? 'Desactivar' : 'Activar recordatorio'}
            </button>
          </div>
        </details>
      </header>
      ...
      <section className="border-y border-slate-100 py-6" aria-labelledby="versiculo-del-dia-title">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-4">
          <Link href={`/biblia?book=${daily.book}&chapter=${daily.chapter}&verse=${daily.verse ?? 1}`} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-[#C0392B] px-4 text-sm font-bold text-white">
      ...
      <section className="mt-10" aria-labelledby="seguir-creciendo-title">
        ...
        <Link href="/hoy/planes" className="mt-2 flex min-h-[86px] items-center justify-between gap-4 border-y border-slate-100 py-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm leading-5 text-slate-500">Elige un tema y avanza con una lectura guiada a tu ritmo.</p>
          </div>
