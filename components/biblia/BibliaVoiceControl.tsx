...
export default function BibliaVoiceControl() {
  ...
  useEffect(() => {
    const abrirOpciones = () => setAbierto(true)
    window.addEventListener(OPEN_BIBLE_VOICE_SETTINGS_EVENT, abrirOpciones)

    const synth = window.speechSynthesis
    if (!synth) {
      return () => window.removeEventListener(OPEN_BIBLE_VOICE_SETTINGS_EVENT, abrirOpciones)
    }
    ...
    return () => {
      window.removeEventListener(OPEN_BIBLE_VOICE_SETTINGS_EVENT, abrirOpciones)
      ...
      <section className="modal-panel-safe flex max-h-[78dvh] w-full max-w-md flex-col rounded-3xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="voice-title">
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
          <div className="min-w-0">
            <h2 id="voice-title" className="font-bold text-slate-900">Opciones de audio</h2>
            <p className="mt-0.5 text-xs text-slate-500">Elige la voz y el estilo para la lectura bíblica.</p>
          </div>
          <button type="button" onClick={() => setAbierto(false)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-500" aria-label="Cerrar opciones de audio">
            <X className="h-5 w-5" />
          </button>
        </header>
          ...
          <section>
            <div className="mb-2 flex items-center justify-between gap-3 px-1">
              ...
              <button
                ...
                className="flex min-h-9 items-center justify-center gap-1.5 rounded-xl bg-indigo-600 px-3 text-center text-xs font-semibold text-white disabled:opacity-50"
              >
