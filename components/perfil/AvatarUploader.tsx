...
async function generarAvatar(editor: EditorState) {
  ...
  const displayWidth = editor.baseWidth * editor.zoom
  const displayHeight = editor.baseHeight * editor.zoom
  const left = (EDITOR_SIZE - displayWidth) / 2 + editor.offsetX
  const top = (EDITOR_SIZE - displayHeight) / 2 + editor.offsetY
  const outputScale = AVATAR_SIZE / EDITOR_SIZE
...
export default function AvatarUploader({ userId, nombre, avatarUrl }: AvatarUploaderProps) {
  ...
  const editorModal = editor && typeof document !== 'undefined'
    ? createPortal(
        <div
          className="fixed inset-0 z-[10030] flex items-end justify-center bg-slate-950/70 px-0 backdrop-blur-sm sm:items-center sm:px-4"
          onClick={(event) => {
            if (event.target === event.currentTarget && !pending) cancelarEditor()
          }}
        >
          <section
            role="dialog"
            ...
            className="w-full max-w-md overflow-hidden rounded-t-[30px] bg-white pb-[max(1rem,env(safe-area-inset-bottom))] shadow-[0_-20px_70px_rgba(15,23,42,0.35)] sm:rounded-[30px] sm:pb-4"
          >
            <header className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 pb-3 pt-[max(1rem,env(safe-area-inset-top))] sm:px-5 sm:pt-4">
              ...
              <button
                ...
                disabled={pending}
                className="grid h-11 w-11 place-items-center rounded-full bg-slate-100 text-slate-600 disabled:opacity-50"
            ...
            <div className="px-4 py-5 sm:px-5">
              ...
              >
                <img
                  ...
                  style={{
                    width: editor.baseWidth * editor.zoom,
                    height: editor.baseHeight * editor.zoom,
                    left: `calc(50% + ${editor.offsetX}px)`,
                    top: `calc(50% + ${editor.offsetY}px)`,
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <button
                  ...
                  onClick={cancelarEditor}
                  disabled={pending}
                  className="min-h-12 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-600 disabled:opacity-50"
                >
                  Cancelar
                </button>
                <button
    ...
    <div className="shrink-0">
      <div className="relative h-16 w-16 sm:h-[72px] sm:w-[72px]">
        <UserAvatar
          ...
          size="xl"
          className="shadow-[0_8px_22px_rgba(15,23,42,0.16)] ring-4 ring-white"
        />
