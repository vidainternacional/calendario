...
function isNarrativeEvent(event: PaqueteCronologicoBiblico['events'][number]) {
  return event.source.name.toLowerCase().includes('theographic')
}

export default function ChronologyMapPanel({ bundle }: { bundle: PaqueteCronologicoBiblico }) {
  if (bundle.events.length === 0) return null

  const narrativeEvents = bundle.events.filter(isNarrativeEvent)
  const chronologyEvents = bundle.events.filter(event => !isNarrativeEvent(event))

  return (
    <div>
      {narrativeEvents.length > 0 && (
        <section className="border-b border-slate-100 bg-violet-50/45 px-5 py-6 sm:px-7" aria-labelledby="narrative-sequence-title">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-violet-700">Índice narrativo verificado</p>
              <h3 id="narrative-sequence-title" className="mt-1 text-lg font-bold text-slate-950">Secuencia narrativa</h3>
              <p className="mt-1 text-sm leading-6 text-slate-600">
                Historias vinculadas exactamente a esta referencia bíblica. El orden es narrativo y no representa una fecha histórica absoluta.
              </p>
            </div>
          </div>
          ...
          <div className="mt-5 space-y-3">
            {narrativeEvents.map(event => (
              <article key={event.slug} className="rounded-2xl border border-violet-200/70 bg-white p-4 shadow-sm">
                <p className="text-xs font-semibold text-violet-700">{bibleRange(event)}</p>
                <h4 className="mt-1 text-base font-bold text-slate-950">{theographicTitleEs(event.title)}</h4>
