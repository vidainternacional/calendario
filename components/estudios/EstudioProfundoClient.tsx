...
const SECTIONS: { key: keyof EstudioResultado; label: string }[] = [
  ...
  { key: 'comparacion_versiones', label: 'Comparación de traducciones' },
...
const PLACEHOLDER_PATTERNS = [
  ...
  'cuando existe un paquete textual',
  'se adjunta por separado',
  'aparece en la evidencia textual',
  'permanece en biblia → comparar',
  'se añadirá cuando exista',
  'no especificado',
...
function languageLabel(language: string) {
  if (language === 'hebrew') return 'Hebreo bíblico'
  if (language === 'aramaic') return 'Arameo bíblico'
  if (language === 'greek') return 'Griego koiné'
  return language
}

function AccordionItem({ item, onCopy }: { item: DashboardItem; onCopy: (key: string, text: string) => void }) {
  return (
...
export default function EstudioProfundoClient({
  ...
  const sectionsWithContent = useMemo(() => {
    ...
    return SECTIONS.filter(section => hasUsefulContent(state.resultado[section.key]))
  }, [state])
  ...
  const dashboardItems = useMemo<DashboardItem[]>(() => {
    ...
    const editions = state.textualEvidence?.editions ?? []

    const originalEditions = editions.filter(edition => Boolean(edition.originalText?.trim()))
    if (originalEditions.length > 0) {
      const plainText = originalEditions
        .map(edition => `${languageLabel(edition.language)}\n${edition.originalText}`)
        .join('\n\n')
      items.push({
        id: 'texto-original-real',
        label: 'Texto original',
        plainText,
        content: (
          <div className="space-y-4">
            {originalEditions.map((edition, index) => (
              <div key={`${edition.language}-${index}`}>
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">{languageLabel(edition.language)}</p>
                <p dir={edition.textDirection} className="mt-2 break-words text-xl leading-9 text-slate-900">{edition.originalText}</p>
              </div>
            ))}
          </div>
        ),
    ...
    const transliterations = editions.filter(edition => Boolean(edition.transliteration?.trim()))
    if (transliterations.length > 0) {
      const plainText = transliterations.map(edition => edition.transliteration || '').filter(Boolean).join('\n\n')
      items.push({
        id: 'transliteracion-real',
        label: 'Transliteración',
        plainText,
        content: <div className="space-y-3 text-[15px] leading-7 text-slate-700">{transliterations.map((edition, index) => <p key={`${edition.language}-${index}`}>{edition.transliteration}</p>)}</div>,
      })
    }

    const literalTranslations = editions.filter(edition => Boolean(edition.literalTranslationEs?.trim()))
    if (literalTranslations.length > 0) {
      const plainText = literalTranslations.map(edition => edition.literalTranslationEs || '').filter(Boolean).join('\n\n')
      items.push({
        id: 'traduccion-literal-real',
        label: 'Traducción literal de estudio',
        plainText,
        content: <div className="space-y-3 text-[15px] leading-7 text-slate-700">{literalTranslations.map((edition, index) => <p key={`${edition.language}-${index}`}>{edition.literalTranslationEs}</p>)}</div>,
      })
    }
    ...
    if (state.textualEvidence) {
      items.push({
        id: 'analisis-textual',
        label: 'Palabra por palabra y evidencia textual',
    ...
    <div className="mx-auto w-full max-w-3xl space-y-5">
      ...
      <form id="estudio-form" action={formAction} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row">
          <div className="relative min-w-0 flex-1">
            ...
            <input type="text" id="pasaje" name="pasaje" required disabled={isPending} placeholder="Ejemplo: Juan 3:16, Daniel 1 o perdón" className="min-h-13 w-full rounded-2xl border border-slate-300 bg-white pl-10 pr-4 text-base text-slate-900 outline-none placeholder:text-slate-400 focus:border-[#C0392B] focus:ring-2 focus:ring-[#C0392B]/20 disabled:opacity-50" defaultValue={initialPasaje} />
          </div>
      ...
      {!isPending && state.status === 'success' && state.kind === 'study' && activeTab === 'study' && (
        <article className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
          <header className="bg-gradient-to-b from-white to-slate-50/70 px-5 pb-5 pt-6 sm:px-7">
            ...
            <p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">Explore solamente las capas que existen para este pasaje. Si una fuente no contiene un dato, VIDA no lo sustituye con explicaciones de relleno.</p>
          ...
          {dashboardItems.length > 0 ? (
            ...
            <div className="border-t border-slate-100 px-5 py-8 text-center text-sm leading-6 text-slate-500 sm:px-7">No hay capas adicionales verificadas para mostrar en esta referencia.</div>
          )}
              ...
              <div className="divide-y divide-slate-100">
                {result.matches.map(match => (
                  <Link key={`${result.termId}-${match.bookCode}-${match.chapter}-${match.verse}-${match.relationKind}`} href={`/biblia?book=${encodeURIComponent(match.bookCode)}&chapter=${match.chapter}&verse=${match.verse}`} className="block px-5 py-4 hover:bg-slate-50 sm:px-6">
                    <div className="flex flex-wrap items-center justify-between gap-2"><h4 className="font-bold text-slate-900">{match.reference}</h4><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-slate-500">{RELATION_LABELS[match.relationKind]}</span></div>
      ...
      {historial.length > 0 && state.status === 'idle' && !isPending && activeTab === 'study' && (
        <details className="rounded-3xl border border-slate-200 bg-white shadow-sm">
          <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-3 px-5 py-4 text-sm font-bold text-slate-700 [&::-webkit-details-marker]:hidden"><span className="flex items-center gap-2"><Clock className="h-4 w-4 text-slate-400" /> Estudios recientes</span><span className="text-xs font-semibold text-slate-400">{historial.length}</span></summary>
          <div className="grid gap-2 border-t border-slate-100 p-4 sm:grid-cols-2">
            {historial.map((item, index) => (
              <button key={`${item.pasaje}-${index}`} type="button" onClick={() => loadQuery(item.pasaje)} className="flex min-h-14 items-center gap-3 rounded-2xl p-3 text-left hover:bg-slate-50"><FileText className="h-4 w-4 shrink-0 text-[#C0392B]" aria-hidden="true" /><span className="min-w-0"><span className="block break-words text-sm font-semibold text-slate-800">{item.pasaje}</span><span className="mt-1 block text-xs text-slate-400">{new Date(item.created_at).toLocaleDateString('es', { day: 'numeric', month: 'short', year: 'numeric' })}</span></span></button>
            ))}
          </div>
        </details>
      )}
