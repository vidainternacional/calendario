  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl bg-[#f4f5f9] px-4 pb-[calc(7rem+env(safe-area-inset-bottom))] pt-[calc(1.25rem+env(safe-area-inset-top))] sm:px-6 sm:pt-8">
      ...
      {tab === 'personas' && <div className="space-y-4">
        <section className={`${panel} p-4`}><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-violet-50 text-violet-600"><UsersRound className="h-5 w-5" /></div><div><h2 className="font-bold text-[#171923]">Personas en el discipulado actual</h2><p className="mt-0.5 text-xs text-slate-500">Ve exactamente en qué punto está cada persona.</p></div></div>{courses.length > 1 && <select value={selectedCourseId || ''} onChange={event => setSelectedCourseId(event.target.value)} className={`${fieldClass} mt-4`}>{courses.map(course => <option key={course.id} value={course.id}>{course.titulo}</option>)}</select>}</section>
        {selectedAssignments.length === 0 ? <section className={`${panel} p-8 text-center`}><BookOpenCheck className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-600">Todavía nadie ha comenzado este curso.</p></section> : selectedAssignments.map(assignment => {
          ...
          return <section key={assignment.id} className={`${panel} p-4 sm:p-5`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-sm font-bold text-[#171923]">{profile?.nombre_completo || 'Miembro'}</h3><p className="mt-1 text-xs text-slate-500">{activity}</p><p className="mt-1 text-[11px] text-slate-400">Última actividad: {new Intl.DateTimeFormat('es-SV',{dateStyle:'medium',timeStyle:'short'}).format(new Date(assignment.updated_at))}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold ring-1 ${statusClass(assignment.estado)}`}>{statusLabel(assignment.estado)}</span></div><div className="mt-4 flex items-center justify-between text-[10px] font-bold uppercase tracking-wider text-slate-400"><span>{completed}/{selectedLessons.length} lecciones</span><span>{percent}%</span></div><div className="mt-1.5 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-violet-600" style={{width:`${percent}%`}} /></div>{assignment.calificacion != null && <p className="mt-3 text-sm font-bold text-[#171923]">Examen final: {Math.round(Number(assignment.calificacion))}%</p>}{assignment.estado === 'revision' && <form className="mt-3 rounded-2xl bg-amber-50 p-3"><textarea name="notas" className={`${fieldClass} min-h-16 resize-y`} placeholder="Nota pastoral opcional" /><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" disabled={busy} onClick={event => void review(assignment,'rechazado',event.currentTarget.form)} className="min-h-10 rounded-xl border border-rose-200 bg-rose-50 text-xs font-bold text-rose-700">Pedir repetir</button><button type="button" disabled={busy} onClick={event => void review(assignment,'aprobado',event.currentTarget.form)} className="min-h-10 rounded-xl bg-emerald-600 text-xs font-bold text-emerald-50">Aprobar</button></div></form>}</section>
        })}
      </div>}
