...
import CalendarioMonthModeGrid from './CalendarioMonthModeGrid'
import {
  eventColor,
  eventosDelDia,
  WEEKDAY_LABELS,
...
function DayReveal({ selectedDay, events, onSelectDay }: { selectedDay: Date; events: EventoCalendario[]; onSelectDay: (day: Date) => void }) {
  const [detail, setDetail] = useState<EventoCalendario | null>(null)
  const touchStart = useRef<number | null>(null)
  const dayEvents = eventosDelDia(events, selectedDay)
  const railDays = useMemo(() => Array.from({ length: 9 }, (_, index) => addDays(selectedDay, index - 4)), [selectedDay])
  ...
  return (
    <section key={format(selectedDay, 'yyyy-MM-dd')} className={flow.dayReveal} onTouchStart={touchStartHandler} onTouchEnd={touchEndHandler} aria-label="Eventos del día seleccionado">
        ...
        {railDays.map((day) => {
          ...
          return (
            <button key={day.toISOString()} className={`${flow.dayChip} ${active ? flow.dayChipActive : ''}`} onClick={() => onSelectDay(day)} aria-pressed={active}>
              ...
              <span className={`${flow.dayChipNumber} ${today ? selection.todayFilled : active ? selection.selectedRing : ''}`}>{format(day, 'd')}</span>
            </button>
          )
        })}
      </div>
        ...
        <div className={flow.eventDetailOverlay} onMouseDown={(event) => event.target === event.currentTarget && setDetail(null)}>
          <article className={flow.eventDetailCard} role="dialog" aria-modal="true" aria-labelledby="month-event-title">
            <header className={flow.eventDetailHeader}>
              <div><h3 id="month-event-title" className={flow.eventDetailTitle}>{detail.titulo}</h3><p className={flow.eventDetailMeta}>{detail.ministerios?.nombre || 'Evento general'}</p></div>
            ...
            <footer className={flow.eventDetailFooter}><button className={flow.eventDetailDone} onClick={() => setDetail(null)}>Listo</button></footer>
          </article>
        </div>,
        document.body,
      )}
...
export default function CalendarioMonthView({
  ...
  selectedDay: Date
  events: EventoCalendario[]
  displayMode?: ModoVisualCalendario
  topChrome: ReactNode
  ...
  const showDayReveal = displayMode === 'detalles' || displayMode === 'lista'
  return (
    <div className={`${styles.calendarScreen} ${flow.monthFlow} ${!overlay ? flow.elasticMonth : ''}`} aria-hidden={overlay || undefined}>
      ...
      <div className={styles.headerBlock}>
        ...
        {!overlay && <p className={styles.subTitle}>Selecciona un día para ver sus eventos{isRefreshing ? ' · Actualizando…' : ''}</p>}
      </div>
      <MonthWeekdayHeader />
      <div className={styles.monthScroll}>
        <section className={styles.monthSection}>
          {overlay
            ...
            : <CalendarioMonthModeGrid month={month} selectedDay={selectedDay} events={events} mode={displayMode} onSelectDay={onSelectDay} />}
        </section>
        {!overlay && showDayReveal && <DayReveal selectedDay={selectedDay} events={events} onSelectDay={onSelectDay} />}
      </div>
    </div>
  )
}
