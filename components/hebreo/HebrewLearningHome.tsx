...
import { Accessibility, ArrowLeft, BookOpenText, ChevronDown, ExternalLink, History, PlayCircle } from 'lucide-react'
...
function MenuButton({ label, open, onClick }: { label: string; open: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} aria-expanded={open} className={`flex min-h-[72px] w-full flex-col items-start justify-between rounded-[22px] border px-4 py-3 text-left transition-all active:scale-[0.98] motion-reduce:transition-none ${open ? 'border-indigo-600 bg-indigo-600 text-white shadow-[0_8px_24px_rgba(79,70,229,0.18)]' : 'border-slate-200 bg-white text-slate-900 shadow-[0_4px_16px_rgba(15,23,42,0.04)]'}`}><span className="text-[13px] font-black leading-tight">{label}</span><ChevronDown className={`h-4 w-4 self-end transition-transform ${open ? 'rotate-180 text-indigo-100' : 'text-slate-400'}`} /></button>
}

function LearningAreaButton({ section, active, onClick }: { section: LearningSection; active: boolean; onClick: () => void }) {
  return <button type="button" aria-expanded={active} aria-controls={`hebrew-panel-${section.id}`} onClick={onClick} className={`flex min-h-[64px] items-center justify-between gap-2 rounded-[20px] border px-3.5 py-2.5 text-left transition-all active:scale-[0.98] motion-reduce:transition-none ${active ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-200 bg-white text-slate-900'}`}><div className="min-w-0"><p lang="he" dir="rtl" className={`text-[1.12rem] font-black leading-none ${active ? 'text-white' : 'text-indigo-700'}`}>{section.he}</p><p className="mt-1 text-[12px] font-black">{section.short}</p></div><ChevronDown className={`h-3.5 w-3.5 shrink-0 transition-transform ${active ? 'rotate-180 text-indigo-100' : 'text-slate-400'}`} /></button>
}
...
function ArchitecturePreview({ section }: { section: LearningSection }) {
  ...
  return <div className="pt-1"><details className="border-y border-slate-200 py-1" open><summary className="flex min-h-11 cursor-pointer list-none items-center justify-between text-sm font-black text-slate-900">Qué aprenderás <ChevronDown className="h-4 w-4 text-slate-400" /></summary><div className="pb-3"><p className="text-sm leading-relaxed text-slate-600">{section.description}</p></div></details><details className="border-b border-slate-200 py-1"><summary className="flex min-h-11 cursor-pointer list-none items-center justify-between text-sm font-black text-indigo-700">Explicación y ejemplo <ChevronDown className="h-4 w-4" /></summary><div className="pb-4"><p lang="he" dir="rtl" className="my-3 text-center text-[3.2rem] font-black leading-none text-slate-950">{examples[section.id]}</p><p className="text-[12px] leading-relaxed text-slate-500">Cada explicación, ejemplo y detalle adicional se abrirá únicamente cuando haga falta.</p></div></details><p className="mt-3 text-[11px] leading-relaxed text-slate-400">Vista de arquitectura del Bloque 1. Todavía no guarda actividad ni desbloquea contenido.</p></div>
}
...
export default function HebrewLearningHome() {
  ...
  return <main className="min-h-screen bg-[#f9f9fb] text-slate-950"><div className="mx-auto w-full max-w-5xl px-4 pb-12 pt-3 sm:px-6"><header className="mb-4"><div className="flex items-center justify-between"><Link href="/estudios" className="inline-flex min-h-10 items-center gap-1.5 text-[13px] font-black text-slate-600"><ArrowLeft className="h-4.5 w-4.5" /> Estudios</Link><button type="button" onClick={() => setSimpleMode(value => !value)} aria-pressed={simpleMode} className={`inline-flex min-h-10 items-center gap-1.5 rounded-full border px-3.5 text-[13px] font-black ${simpleMode ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-200 bg-white text-slate-700'}`}><Accessibility className="h-4 w-4" /> Lectura</button></div><div className="mt-4 text-center"><p lang="he" dir="rtl" className="text-[2.6rem] font-black leading-none text-indigo-700">עברית מקראית</p><h1 className="mt-1.5 text-[1.75rem] font-black tracking-[-0.035em]">Hebreo Bíblico</h1><p className="mt-0.5 text-[13px] font-semibold text-slate-500">Aprende a leer paso a paso.</p></div></header>

<section className="grid grid-cols-2 gap-2.5" aria-label="Navegación principal de Hebreo Bíblico">
  <MenuButton label="Aprender" open={openMenu === 'learn'} onClick={() => setOpenMenu(current => current === 'learn' ? null : 'learn')} />
  <MenuButton label="Materiales y curso" open={openMenu === 'materials'} onClick={() => setOpenMenu(current => current === 'materials' ? null : 'materials')} />
  <MenuButton label="Prueba tu progreso" open={openMenu === 'test'} onClick={() => setOpenMenu(current => current === 'test' ? null : 'test')} />
  <MenuButton label="Biblia en hebreo" open={openMenu === 'bible'} onClick={() => setOpenMenu(current => current === 'bible' ? null : 'bible')} />
</section>

{openMenu && <section className="mt-5 border-t border-slate-200 pt-4">{openMenu === 'learn' && <LearnPanel simpleMode={simpleMode} />}{openMenu === 'materials' && <SupportMaterialSection />}{openMenu === 'test' && <div><div><h2 className="text-lg font-black">Comprueba lo aprendido</h2><p className="mt-0.5 text-[12px] leading-relaxed text-slate-500">15 preguntas variadas ofrecen una base más amplia para evaluar tu proceso.</p></div><ProcessTestPreview /><details className="mt-4 border-y border-slate-200 py-1"><summary className="flex min-h-11 cursor-pointer list-none items-center justify-between text-sm font-black text-slate-700">Historial de progreso <History className="h-4 w-4 text-slate-400" /></summary><p className="pb-4 text-[12px] leading-relaxed text-slate-500">Aquí se consultarán tus fichas anteriores para comparar resultados y recomendaciones. No existe persistencia durante Bloque 1.</p></details></div>}{openMenu === 'bible' && <BibleHebrewPreview />}</section>}

<footer className="mt-8 border-t border-slate-200 pt-4"><p className="text-[10px] leading-relaxed text-slate-400">FASE H · Bloque 1 · Arquitectura visual. Sin audio, progreso persistente ni desbloqueos automáticos.</p></footer></div></main>
}
