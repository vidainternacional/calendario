...
function LearningAreaButton({ section, active, onClick }: { section: LearningSection; active: boolean; onClick: () => void }) {
  return <button type="button" aria-expanded={active} aria-controls={`hebrew-panel-${section.id}`} onClick={onClick} className={`flex min-h-[66px] flex-col items-center justify-center rounded-[20px] border px-2.5 py-2 text-center transition-all active:scale-[0.98] motion-reduce:transition-none ${active ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-200 bg-white text-slate-900'}`}><p lang="he" dir="rtl" className={`text-[1.08rem] font-black leading-none ${active ? 'text-white' : 'text-indigo-700'}`}>{section.he}</p><p className="mt-1 text-[12px] font-black">{section.short}</p><ChevronDown className={`mt-1 h-3 w-3 transition-transform ${active ? 'rotate-180 text-indigo-100' : 'text-slate-400'}`} /></button>
}

function ArchitecturePreview({ section }: { section: LearningSection }) {
  return <div className="pt-1 text-center"><details className="border-y border-slate-200 py-1" open><summary className="flex min-h-11 cursor-pointer list-none items-center justify-center gap-2 text-sm font-black text-slate-900">Qué aprenderás <ChevronDown className="h-4 w-4 text-slate-400" /></summary><div className="pb-3"><p className="mx-auto max-w-xl text-sm leading-relaxed text-slate-600">{section.description}</p></div></details><details className="border-b border-slate-200 py-1"><summary className="flex min-h-11 cursor-pointer list-none items-center justify-center gap-2 text-sm font-black text-indigo-700">Ejemplo <ChevronDown className="h-4 w-4" /></summary><div className="pb-4"><p lang="he" dir="rtl" className="my-3 text-[3.2rem] font-black leading-none text-slate-950">{section.example}</p></div></details><details className="border-b border-slate-200 py-1"><summary className="flex min-h-11 cursor-pointer list-none items-center justify-center gap-2 text-sm font-black text-indigo-700">En qué enfocarte <ChevronDown className="h-4 w-4" /></summary><p className="mx-auto max-w-xl pb-4 text-[13px] leading-relaxed text-slate-600">{section.focus}</p></details><details className="border-b border-slate-200 py-1"><summary className="flex min-h-11 cursor-pointer list-none items-center justify-center gap-2 text-sm font-black text-indigo-700">Cómo practicar <ChevronDown className="h-4 w-4" /></summary><p className="mx-auto max-w-xl pb-4 text-[13px] leading-relaxed text-slate-600">{section.practice}</p></details><p className="mt-3 text-[11px] leading-relaxed text-slate-400">Vista de arquitectura del Bloque 1. Todavía no guarda actividad ni desbloquea contenido.</p></div>
}
