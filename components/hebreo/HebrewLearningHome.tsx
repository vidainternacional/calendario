...
function QuickButton({ id, icon, title, subtitle, active, onToggle }: { id: QuickPanelId; icon: React.ReactNode; title: string; subtitle: string; active: boolean; onToggle: (id: QuickPanelId) => void }) {
  return <button type="button" onClick={() => onToggle(id)} aria-expanded={active} className={`flex min-h-[96px] flex-col items-center justify-center rounded-[20px] px-2 py-3 text-center transition active:scale-[0.98] ${active ? 'bg-indigo-600 text-white shadow-[0_10px_25px_rgba(79,70,229,0.2)]' : 'bg-white text-slate-950 shadow-sm ring-1 ring-slate-200/80'}`}><span className={`grid h-9 w-9 place-items-center rounded-[13px] ${active ? 'bg-white/15 text-white' : 'bg-indigo-50 text-indigo-700'}`}>{icon}</span><span className="mt-2 text-[12px] font-black leading-tight">{title}</span><span className={`mt-0.5 text-[9px] font-semibold leading-tight ${active ? 'text-indigo-100' : 'text-slate-400'}`}>{subtitle}</span></button>
}
...
function PracticeTab({ id, icon, active, onToggle }: { id: PracticePanelId; icon: React.ReactNode; active: boolean; onToggle: (id: PracticePanelId) => void }) {
  ...
  return <button type="button" onClick={() => onToggle(id)} aria-expanded={active} aria-label={panel.title} className={`flex min-h-[72px] min-w-0 flex-col items-center justify-center rounded-[16px] px-1 py-2 text-center transition active:scale-[0.98] ${active ? 'bg-indigo-600 text-white shadow-[0_8px_20px_rgba(79,70,229,0.18)]' : 'bg-white text-slate-700 ring-1 ring-slate-200'}`}><span className={`grid h-8 w-8 place-items-center rounded-[11px] ${active ? 'bg-white/15 text-white' : 'bg-indigo-50 text-indigo-700'}`}>{icon}</span><span className="mt-1.5 block max-w-full truncate text-[9px] font-black leading-tight">{panel.shortTitle}</span></button>
}
...
export default function HebrewLearningHome() {
  ...
  return <main className="min-h-screen bg-[#f9f9fb] text-slate-950"><div className="mx-auto w-full max-w-xl px-4 pb-8 pt-3 sm:px-6">
    ...
    <section aria-label="Comenzar a aprender" className="mt-5"><Link href="/estudios/hebreo/aprender" className="group relative flex min-h-[104px] items-center justify-center overflow-hidden rounded-[26px] bg-indigo-600 px-5 py-4 text-center text-white shadow-[0_14px_35px_rgba(79,70,229,0.22)] transition active:scale-[0.985]"><span aria-hidden="true" lang="he" dir="rtl" className="hebrew-glimmer pointer-events-none absolute inset-0 flex items-center justify-around px-4 text-[2.4rem] font-black tracking-[0.18em] text-white/[0.07]">א ב ג ד ה</span><span className="relative z-10 text-[21px] font-black tracking-[-0.02em]">Empecemos</span></Link></section>
    ...
    {openQuick && <section aria-label={`${QUICK_PANELS[openQuick].title} desplegado`} className="mt-5 border-t border-slate-200 pt-5">{openQuick === 'translator' && <div><div className="mb-4 text-center"><p lang="he" dir="rtl" className="text-[1.05rem] font-black text-indigo-700">תַּרְגּוּם</p><h2 className="mt-1 text-[1.35rem] font-black">Traductor</h2><p className="mt-1 text-[12px] text-slate-500">Escribe una palabra o frase sin salir del inicio.</p></div><HebrewTranslator embedded /></div>}{openQuick === 'bible' && <HebrewBibleReader />}{openQuick === 'materials' && <div><div className="mb-4 text-center"><p lang="he" dir="rtl" className="text-[1.05rem] font-black text-indigo-700">חֹמֶר לִמּוּד</p><h2 className="mt-1 text-[1.35rem] font-black">Materiales</h2><p className="mt-1 text-[12px] text-slate-500">Recursos del curso organizados por etapas.</p></div><HebrewSupportMaterials embedded /></div>}</section>}
    <section aria-label="Práctica" className="mt-5 border-t border-slate-200"><button type="button" onClick={() => setProgressOpen(value => !value)} aria-expanded={progressOpen} className="relative flex min-h-[76px] w-full items-center justify-center px-12 text-center"><span><span className="block text-[14px] font-black text-slate-900">Prueba tu progreso</span><span className="mt-0.5 block text-[10px] text-slate-400">Entrena a tu ritmo</span></span><ChevronDown className={`absolute right-3 h-4 w-4 text-slate-400 transition-transform ${progressOpen ? 'rotate-180' : ''}`} /></button>
      {progressOpen && <div className="border-t border-slate-100 pt-3"><nav aria-label="Secciones de práctica" className="grid grid-cols-4 gap-2"><PracticeTab id="evaluation" icon={<BarChart3 className="h-4 w-4" />} active={openPractice === 'evaluation'} onToggle={togglePractice} /><PracticeTab id="speech" icon={<Mic className="h-4 w-4" />} active={openPractice === 'speech'} onToggle={togglePractice} /><PracticeTab id="keyboard" icon={<Keyboard className="h-4 w-4" />} active={openPractice === 'keyboard'} onToggle={togglePractice} /><PracticeTab id="progress" icon={<Gauge className="h-4 w-4" />} active={openPractice === 'progress'} onToggle={togglePractice} /></nav>{openPractice && <div className="mt-4 border-t border-slate-100 pt-3">{openPractice === 'evaluation' && <div data-evaluation-only className="px-1 pb-5"><HebrewProgressCoach /></div>}{openPractice === 'speech' && <div className="pb-4"><HebrewSpeechPractice /></div>}{openPractice === 'keyboard' && <div className="pb-5"><HebrewKeyboardDock enabled /></div>}{openPractice === 'progress' && <div className="px-1 pb-4"><HebrewProgressSummary /></div>}</div>}</div>}
    </section>
  ...
  </div><style jsx global>{`@keyframes hebrewGlimmer { 0%,72%,100%{opacity:.35;filter:brightness(1)}80%{opacity:.72;filter:brightness(2.3)}88%{opacity:.35;filter:brightness(1)}} .hebrew-glimmer{animation:hebrewGlimmer 7s ease-in-out infinite} [data-evaluation-only] .mt-5.border-t.border-slate-200.pt-2.text-center{display:none} @media(prefers-reduced-motion:reduce){.hebrew-glimmer{animation:none}}`}</style></main>
}
