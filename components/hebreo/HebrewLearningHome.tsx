...
import { ArrowLeft, BookOpenText, ChevronDown, ExternalLink, History, Keyboard, PlayCircle } from 'lucide-react'
...
const SECTIONS: readonly LearningSection[] = [
  ...
  { id: 'reading', he: 'קְרִיאָה', short: 'Lectura', description: 'Lee frases y oraciones reales del hebreo bíblico, desde expresiones cortas hasta versículos completos.', example: 'בְּרֵאשִׁית בָּרָא', focus: 'Mantener continuidad al leer varias palabras juntas y depender cada vez menos de las ayudas.', practice: 'Frases iniciales → oraciones cortas → medias → largas → corpus completo.', available: true },
  { id: 'grammar', he: 'דִּקְדּוּק', short: 'Reglas', description: 'Comprende patrones esenciales por capas, combinando tablas, fichas, transformaciones y lectura aplicada.', example: 'הַ  ·  וְ  ·  בְּ', focus: 'Reconocer piezas frecuentes al inicio y al final de las palabras antes de entrar al sistema verbal.', practice: 'Tabla → ficha → transformación → ejemplo → lectura aplicada.', available: true },
  { id: 'review', he: 'חֲזָרָה', short: 'Repaso', description: 'Recupera lo estudiado mediante sesiones breves y mezcladas, sin convertirlo en otro examen.', example: 'ב  ·  לַ  ·  מֶלֶךְ', focus: 'Distinguir lo que ya reconoces de lo que conviene volver a practicar.', practice: 'Hasta ocho elementos → revelar respuesta → Lo sé / Necesito practicar / Repasar después.', available: true },
]
...
function MenuButton({ id, open, onClick }: { id: TopMenuId; open: boolean; onClick: () => void }) {
  ...
  return (
    <button type="button" onClick={onClick} aria-expanded={open} className={`flex min-h-[74px] w-full flex-col items-center justify-center rounded-[22px] border px-3 py-2.5 text-center transition-all active:scale-[0.98] motion-reduce:transition-none ${open ? 'border-indigo-600 bg-indigo-600 text-white shadow-[0_8px_24px_rgba(79,70,229,0.18)]' : 'border-slate-200 bg-white text-slate-900 shadow-[0_4px_16px_rgba(15,23,42,0.04)]'}`}>
      ...
      <ChevronDown className={`mt-1 h-3.5 w-3.5 transition-transform ${open ? 'rotate-180 text-indigo-100' : 'text-slate-400'}`} />
    </button>
  )
}
...
function LearningAreaButton({ section, active, onClick }: { section: LearningSection; active: boolean; onClick: () => void }) {
  return (
    <button type="button" aria-expanded={active} aria-controls={`hebrew-panel-${section.id}`} onClick={onClick} className={`flex min-h-[66px] flex-col items-center justify-center rounded-[20px] border px-2.5 py-2 text-center transition-all active:scale-[0.98] motion-reduce:transition-none ${active ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-200 bg-white text-slate-900'}`}>
      ...
      <ChevronDown className={`mt-1 h-3 w-3 transition-transform ${active ? 'rotate-180 text-indigo-100' : 'text-slate-400'}`} />
    </button>
  )
}
...
function KeyboardToolToggle({ enabled, onToggle }: { enabled: boolean; onToggle: () => void }) {
  return (
    <div className="mt-3 border-y border-slate-200">
      <button type="button" onClick={onToggle} aria-pressed={enabled} className="flex min-h-14 w-full items-center justify-between gap-3 py-2 text-left">
        <span className="flex min-w-0 items-center gap-3">
          <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-[14px] ${enabled ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-indigo-700'}`}><Keyboard className="h-5 w-5" aria-hidden="true" /></span>
          <span className="min-w-0">
            <span lang="he" dir="rtl" className="block text-[12px] font-black text-indigo-700">מִקְלֶדֶת עִבְרִית</span>
            <span className="block text-[13px] font-black text-slate-900">Teclado hebreo</span>
            <span className="block text-[10px] font-semibold text-slate-400">Opcional para búsqueda y práctica de escritura</span>
          </span>
        </span>
        <span className={`shrink-0 rounded-full px-3 py-2 text-[10px] font-black ${enabled ? 'bg-indigo-50 text-indigo-700' : 'bg-slate-100 text-slate-500'}`}>{enabled ? 'Desactivar' : 'Activar'}</span>
      </button>
    </div>
  )
}
  ...
  return (
    <div className="border-b border-slate-200">
      <button type="button" onClick={() => setOpen(value => !value)} aria-expanded={open} className="flex min-h-12 w-full flex-col items-center justify-center px-3 py-2 text-center">
        ...
        <span className="mt-0.5 flex items-center gap-1.5 text-sm font-black text-slate-900">{es}<ChevronDown className={`h-4 w-4 text-slate-400 transition-transform ${open ? 'rotate-180' : ''}`} /></span>
      </button>
      {open && <section className="border-t border-slate-100 p-4"><div className="max-h-72 overflow-y-auto overscroll-contain pr-1 [-webkit-overflow-scrolling:touch]">{children}</div></section>}
    </div>
  )
}
...
function ArchitecturePreview({ section }: { section: LearningSection }) {
  return (
    <div className="border-t border-slate-200 text-center">
      <ScrollableDisclosure he="מַה נִלְמַד" es="Qué aprenderás"><p className="mx-auto max-w-xl text-sm leading-relaxed text-slate-600">{section.description}</p></ScrollableDisclosure>
      <ScrollableDisclosure he="דֻּגְמָה" es="Ejemplo"><p lang="he" dir="rtl" className="my-4 text-[3.2rem] font-black leading-tight text-slate-950">{section.example}</p></ScrollableDisclosure>
      <ScrollableDisclosure he="בַּמֶּה לְהִתְמַקֵּד" es="En qué enfocarte"><p className="mx-auto max-w-xl text-[13px] leading-relaxed text-slate-600">{section.focus}</p></ScrollableDisclosure>
      <ScrollableDisclosure he="אֵיךְ לְתַרְגֵּל" es="Cómo practicar"><p className="mx-auto max-w-xl text-[13px] leading-relaxed text-slate-600">{section.practice}</p></ScrollableDisclosure>
    ...
    <section className="text-center" aria-labelledby="areas-hebreo-title">
      ...
      {activeSection && (
        <div id={`hebrew-panel-${activeSection.id}`} className="mt-4 border-t border-slate-200 pt-4">
          {activeSection.id === 'alef-bet' ? <AlefBetExplorer simpleMode={false} />
            : activeSection.id === 'vowels' ? <NiqqudExplorer />
              : activeSection.id === 'vocabulary' ? <ReadingWordsExplorer />
                : activeSection.id === 'reading' ? <ReadingSentencesExplorer />
                  : activeSection.id === 'grammar' ? <GrammarExplorer />
                    : activeSection.id === 'review' ? <ReviewExplorer />
  ...
  if (finished) {
    return (
      <section className="mt-4 rounded-[26px] border border-slate-200 bg-white p-5 text-center shadow-[0_16px_40px_rgba(15,23,42,0.09)]" aria-labelledby="resultado-prueba-title">
        ...
        <div className="mt-5 flex justify-center gap-3 border-t border-slate-200 pt-4 text-[12px] leading-relaxed text-slate-500"><History className="mt-0.5 h-4 w-4 shrink-0" /><p>Cuando exista persistencia autorizada, cada ficha quedará en Historial para comparar pruebas anteriores.</p></div>
      ...
      <div className="mt-3 space-y-2">
        {groups.map(group => (
          <details key={group.title} className="rounded-[18px] border border-slate-200 bg-white">
            <summary className="flex min-h-12 cursor-pointer list-none items-center justify-center gap-2 px-4 text-sm font-black text-slate-900">{group.title} · Clases {group.range[0]}–{group.range[1]} <ChevronDown className="h-4 w-4 text-slate-400" /></summary>
            <div className="max-h-72 space-y-3 overflow-y-auto border-t border-slate-100 p-3 [-webkit-overflow-scrolling:touch]">
              {HEBREW_SUPPORT_COURSE.filter(item => item.orden >= group.range[0] && item.orden <= group.range[1]).map(item => (
                <article key={item.orden} className="overflow-hidden rounded-[16px] border border-slate-200 bg-white text-left">
                  <div className="flex gap-3 p-3"><div className="relative h-[68px] w-[106px] shrink-0 overflow-hidden rounded-[11px] bg-slate-100"><img src={item.miniatura} alt={`Miniatura del enlace de la clase ${item.orden}`} className="h-full w-full object-cover" loading="lazy" /><span className="absolute inset-0 grid place-items-center bg-slate-950/10"><PlayCircle className="h-7 w-7 text-white" /></span></div><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[0.08em] text-amber-700">Clase {item.orden} · Pendiente</p><h3 className="mt-1 text-[13px] font-black leading-snug text-slate-900">{item.titulo}</h3><p className="mt-1 text-[11px] text-slate-500">{item.tema}</p></div></div>
  ...
  return (
    <section className="text-center">
      <div className="mx-auto max-w-xl">
        ...
        <div className="mt-4 flex items-start justify-center gap-3 rounded-[18px] bg-white p-4 text-[11px] leading-relaxed text-slate-500"><BookOpenText className="mt-0.5 h-4 w-4 shrink-0 text-indigo-700" /><p>Esta vista define la experiencia. Durante FASE H no crea otro motor bíblico ni nuevas tablas.</p></div>
      </div>
    </section>
  )
}
    ...
    <main className="min-h-screen bg-[#f9f9fb] text-slate-950">
      <div className="mx-auto w-full max-w-5xl px-4 pb-12 pt-3 sm:px-6">
        <header className="mb-4">
          <Link href="/estudios" className="inline-flex min-h-10 items-center gap-1.5 text-[13px] font-black text-slate-600"><ArrowLeft className="h-4.5 w-4.5" /> Estudios</Link>
          <div className="mt-3 text-center"><p lang="he" dir="rtl" className="text-[2.6rem] font-black leading-none text-indigo-700">עברית מקראית</p><h1 className="mt-1.5 text-[1.75rem] font-black tracking-[-0.035em]">Hebreo Bíblico</h1><p className="mt-0.5 text-[13px] font-semibold text-slate-500">Aprende hebreo bíblico paso a paso.</p></div>
        </header>
