...
function ReadingCard({ item, mode, open, onToggle }: { item: HebrewReadingItem; mode: ReadingMode; open: boolean; onToggle: () => void }) {
  return <article className="overflow-hidden rounded-[24px] border border-slate-200 bg-white"><button type="button" aria-expanded={open} onClick={onToggle} className="block w-full px-4 py-4 text-center"><p className="text-[11px] font-black text-indigo-700">{item.reference}</p><HebrewText item={item} mode={mode} className="mt-2 block break-words text-[2rem] font-black leading-[1.65] text-slate-950" /><p className="mt-2 break-words text-[12px] font-bold leading-relaxed text-slate-500">{item.transliteration ?? 'Pronunciación pendiente'}</p></button>{open && <div className="border-t border-slate-100 px-4 py-4 text-center"><p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">En español</p><p className="mt-2 text-[14px] font-bold leading-relaxed text-slate-800">{item.spanish ?? 'Comparación española no disponible para esta referencia.'}</p></div>}</article>
}
...
function ListView({ items, mode }: { items: HebrewReadingItem[]; mode: ReadingMode }) {
  return <div className="divide-y divide-slate-200 border-y border-slate-200">{items.map(item => <div key={item.id} className="py-3"><div className="flex items-start justify-between gap-3"><p className="shrink-0 text-[11px] font-black text-indigo-700">{item.reference}</p><p className="max-w-[42%] text-right text-[11px] font-bold leading-snug text-slate-500">{item.spanish ?? '—'}</p></div><HebrewText item={item} mode={mode} className="mt-2 block break-words text-right text-[1.65rem] font-black leading-[1.55] text-slate-950" /></div>)}</div>
}

function DetailView({ item, mode }: { item: HebrewReadingItem; mode: ReadingMode }) {
  return <article className="rounded-[28px] border border-slate-200 bg-white p-5 text-center shadow-[0_16px_42px_rgba(15,23,42,0.08)]"><p className="text-[12px] font-black text-indigo-700">{item.reference}</p><HebrewText item={item} mode={mode} className="mt-5 block break-words text-[2.7rem] font-black leading-[1.65] text-slate-950" /><div className="mt-5 divide-y divide-slate-200 border-y border-slate-200 text-left"><div className="py-4"><p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Cómo se pronuncia</p><p className="mt-1 break-words text-[14px] font-bold leading-relaxed text-indigo-700">{item.transliteration ?? 'Pronunciación pendiente'}</p></div><div className="py-4"><p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">En español</p><p className="mt-1 text-[14px] font-bold leading-relaxed text-slate-800">{item.spanish ?? 'Comparación española no disponible para esta referencia.'}</p></div></div></article>
}
