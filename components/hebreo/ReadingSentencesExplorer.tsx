...
const GROUPS: readonly { id: HebrewReadingGroup; label: string; description: string }[] = [
  { id: 'starter', label: 'Iniciales', description: 'Empieza con pasajes reconocibles del curso, incluido el Shemá, antes de ampliar la longitud.' },
...
function CourseReadingMilestone() {
  return <section className="rounded-[20px] bg-indigo-50 px-4 py-3 text-center"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-indigo-500">Hito de lectura del curso</p><p lang="he" dir="rtl" className="mt-1 text-[1.5rem] font-black text-indigo-950">שְׁמַע יִשְׂרָאֵל</p><p className="mt-1 text-[12px] font-black text-indigo-800">Shemá Yisrael · Deuteronomio 6:4–5</p><p className="mt-1 text-[11px] leading-relaxed text-indigo-700/80">Practícalo con niqqud, luego sin niqqud. La meta es leer el pasaje, no memorizar una transliteración.</p></section>
}

function Introduction() {
  const [open, setOpen] = useState(false)
  return <section className="border-y border-slate-200 text-left"><button type="button" onClick={() => setOpen(value => !value)} aria-expanded={open} className="flex min-h-12 w-full items-center justify-between gap-3 py-2"><span><span lang="he" dir="rtl" className="block text-[12px] font-black text-indigo-700">קְרִיאָה</span><span className="mt-0.5 block text-sm font-black text-slate-950">¿Cómo practicar lectura?</span></span><ChevronDown className={`h-4 w-4 text-slate-400 transition-transform ${open ? 'rotate-180' : ''}`} /></button>{open && <div className="border-t border-slate-200 p-4"><div className="max-h-72 space-y-3 overflow-y-auto overscroll-contain pr-1 text-[14px] leading-relaxed text-slate-600 [-webkit-overflow-scrolling:touch]"><p>Lee de derecha a izquierda y sigue el método del curso: consonante → signo vocálico → sílaba → palabra → frase.</p><p>Empieza con <strong>Con niqqud</strong>. Cuando una lectura se vuelva familiar, pásala a <strong>Sin niqqud</strong> e intenta reconocerla solo por sus letras.</p><p>Dedica unos minutos diarios a leer en voz alta. Las lecturas iniciales incluyen el Shemá Yisrael como hito aplicado antes de pasar a textos más largos.</p></div></div>}</section>
}
