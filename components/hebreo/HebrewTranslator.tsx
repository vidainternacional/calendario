          ...
          <div className="overflow-hidden">
            <div className="border-t border-white/75 px-4 pb-4 pt-4">
              ...
              <form onSubmit={translate}>
                <div className="mb-2 flex items-center justify-between gap-3"><label htmlFor="hebrew-translation-input" className="text-[12px] font-black text-slate-600">Palabra o frase</label><span className="text-[11px] font-black text-indigo-700">{inputIsHebrew ? 'Hebreo → Español' : 'Español → Hebreo'}</span></div>
              ...
              <div className="mt-4 min-h-4" aria-live="polite">
                ...
                {result && <div className="overflow-hidden rounded-[22px] border border-white/80 bg-white/52 backdrop-blur-2xl"><div className="flex items-center justify-between gap-3 border-b border-white/70 px-4 py-3"><p className="text-[10px] font-black uppercase tracking-[0.09em] text-slate-400">{result.kind === 'word' ? 'Significado' : 'Traducción'}</p><p className="text-[10px] font-bold text-slate-400">{result.sourceLanguage === 'he' ? 'Hebreo → Español' : 'Español → Hebreo'}</p></div><div className="px-4 py-5"><p lang={outputIsHebrew ? 'he' : 'es'} dir={outputIsHebrew ? 'rtl' : 'ltr'} className={`break-words font-black tracking-[-0.02em] text-slate-950 ${outputIsHebrew ? 'text-[2.45rem] leading-[1.45]' : 'text-[1.6rem] leading-snug'}`}>{visibleResult}</p></div><div className="grid grid-cols-2 gap-2 border-t border-white/70 p-2.5"><button type="button" onClick={copyResult} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[16px] bg-slate-100/72 px-3 text-xs font-black text-slate-700">{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{copied ? 'Copiado' : 'Copiar'}</button><button type="button" onClick={speakHebrew} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[16px] bg-indigo-500/10 px-3 text-xs font-black text-indigo-700"><Speaker className="h-4 w-4" />Escuchar</button></div></div>}
              </div>
            </div>
          </div>
        </div>
