...
function LearningDetail({ word, mode, compact = false }: { word: CatalogWord; mode: ReadingMode; compact?: boolean }) {
  ...
  return <article className={`${compact ? 'mt-3' : ''} overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_16px_42px_rgba(15,23,42,0.08)]`}><div className="p-5 text-center"><WordText word={word} mode={mode} className={`${compact ? 'text-[4.15rem]' : 'text-[5rem]'} block break-words font-black leading-[1.3] text-slate-950`} /><p className="mt-2 text-[1rem] font-black text-indigo-700">{pronunciation}</p><p className="mt-1 text-xl font-black text-slate-950">{spanish}</p><div className="mt-5 divide-y divide-slate-200 border-y border-slate-200 text-left"><div className="py-4"><p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Cómo se pronuncia</p><p className="mt-1 text-[15px] font-black text-slate-800">{pronunciation}</p></div><div className="py-4"><p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Cómo se forma</p><p className="mt-1 text-[13px] leading-relaxed text-slate-700">{formation.length > 0 ? formation.join(' + ') : 'Se conserva la escritura hebrea aprobada de la palabra.'}</p></div><div className="py-4"><p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Qué significa</p><p className="mt-1 text-[14px] font-bold leading-relaxed text-slate-800">{word.meaningNoteEs ?? (word.spanish ? `Significa «${word.spanish}».` : 'El significado español de esta entrada todavía no está preparado editorialmente.')}</p></div></div></div></article>
}
...
function CardsView({ words, mode, selectedId, closingId, onToggle }: { words: CatalogWord[]; mode: ReadingMode; selectedId: string | null; closingId: string | null; onToggle: (word: CatalogWord) => void }) {
  ...
  return <div className="space-y-4">{rows.map((row, rowIndex) => { const selected = row.find(word => word.lexicalId === selectedId) ?? null; return <div key={`word-row-${rowIndex}`}><div className="grid grid-cols-2 gap-3">{row.map(word => { const active = word.lexicalId === selectedId; return <button key={word.lexicalId} type="button" aria-pressed={active} onClick={() => onToggle(word)} className={`min-h-[142px] min-w-0 rounded-[22px] border px-3 py-3 text-center transition active:scale-[0.98] motion-reduce:transition-none ${active ? 'border-indigo-500 bg-indigo-600 text-white shadow-[0_10px_26px_rgba(79,70,229,0.18)]' : 'border-slate-200 bg-white text-slate-950'}`}><WordText word={word} mode={mode} className="block break-words text-[2.5rem] font-black leading-tight" /><span className={`mt-2 block text-[12px] font-black ${active ? 'text-indigo-100' : 'text-indigo-700'}`}>{pronunciationFor(word)}</span><span className={`mt-1 block break-words text-[13px] font-black leading-tight ${active ? 'text-white' : 'text-slate-800'}`}>{spanishFor(word)}</span></button>})}</div>{selected && <div className={closingId === selected.lexicalId ? 'scale-[0.96] opacity-0 transition duration-150' : 'scale-100 opacity-100 transition duration-200'}><LearningDetail word={selected} mode={mode} compact /></div>}</div> })}</div>
}

function ListView({ words, mode }: { words: CatalogWord[]; mode: ReadingMode }) {
  return <div className="divide-y divide-slate-200 border-y border-slate-200">{words.map(word => <div key={word.lexicalId} className="flex min-h-[76px] items-center justify-between gap-4 py-3"><div className="min-w-0"><WordText word={word} mode={mode} className="block break-words text-[2rem] font-black leading-tight text-slate-950" /><p className="mt-1 text-[12px] font-black text-indigo-700">{pronunciationFor(word)}</p></div><p className="max-w-[44%] shrink-0 text-right text-[13px] font-black leading-tight text-slate-800">{spanishFor(word)}</p></div>)}</div>
}
...
export default function ReadingWordsExplorer() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/estudios/hebreo/palabras?${params.toString()}`, { cache: 'no-store', signal: controller.signal })
      ...
      .then(data => { if (!active) return; setResult(data); setSelectedId(current => data.items.some(item => item.lexicalId === current) ? current : null); setClosingId(null) })
