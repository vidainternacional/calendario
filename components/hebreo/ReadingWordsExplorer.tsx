  ...
  return <article className="mt-3 overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-[0_12px_32px_rgba(15,23,42,0.07)]">
    <div className="p-4 text-center">
      ...
      <div className="mt-4 divide-y divide-slate-200 border-y border-slate-200 text-left">
        ...
        <div className="py-3.5"><p className="text-[10px] font-black uppercase text-slate-400">Qué significa</p><p className="mt-1 text-[14px] font-bold text-slate-800">{word.meaningNoteEs??`Significa «${spanish}».`}</p></div>
      </div>
    </div>
  </article>
}

function CardsView({words,mode,selectedId,onToggle}:{words:CatalogWord[];mode:ReadingMode;selectedId:string|null;onToggle:(w:CatalogWord)=>void}){
...
export default function ReadingWordsExplorer(){
  ...
  function changePage(next:number){if(next<1||next>result.totalPages||loading)return;setPage(next);setSelectedId(null)}
  ...
  return <section aria-labelledby="reading-words-title" className="text-left">
    ...
    <div className="mt-4 grid grid-cols-2 rounded-[17px] bg-slate-100 p-1">{(['nikud','plain'] as const).map(item=><button key={item} type="button" onClick={()=>setMode(item)} className={`min-h-10 rounded-[14px] text-[12px] font-black ${mode===item?'bg-white text-indigo-700 shadow-sm':'text-slate-500'}`}>{item==='nikud'?'Con niqqud':'Sin niqqud'}</button>)}</div>
    ...
    <div className="mt-3 flex items-center justify-between text-[11px] text-slate-500"><span>{searchResult?`${searchResult.total} coincidencias exactas`:(loading?'Cargando palabras…':`${result.total.toLocaleString('es-SV')} palabras bíblicas`)}</span>{searchResult&&<button type="button" onClick={clearSearch} className="font-black text-indigo-700">Volver al catálogo</button>}</div>
    ...
    {!error&&!noResults&&visibleWords.length>0&&<>
      ...
      <div className="mt-5 touch-pan-y" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        {view==='cards'&&<CardsView words={visibleWords} mode={mode} selectedId={selectedId} onToggle={toggleCard}/>} 
