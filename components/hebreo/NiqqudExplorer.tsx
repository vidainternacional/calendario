...
function NiqqudTile({ mark, selected, onSelect }: { mark: NiqqudMark; selected: boolean; onSelect: () => void }) {
  return <button type="button" aria-pressed={selected} onClick={onSelect} className={`min-h-[132px] min-w-0 rounded-[22px] border px-2.5 pb-3 pt-3 text-center transition active:scale-[0.98] motion-reduce:transition-none ${selected ? 'border-indigo-500 bg-indigo-600 text-white shadow-[0_10px_26px_rgba(79,70,229,0.18)]' : 'border-slate-200 bg-white text-slate-950'}`}><span className={`block text-[10px] font-black uppercase tracking-[0.1em] ${selected ? 'text-white/70' : 'text-slate-400'}`}>{mark.family}</span><span lang="he" dir="rtl" className="mt-1 block text-[3.75rem] font-black leading-none" aria-hidden="true">{mark.example}</span><span className="mt-2 block break-words text-[13px] font-black leading-tight">{mark.name}</span><span className={`mt-1 block text-[12px] font-bold ${selected ? 'text-indigo-100' : 'text-slate-500'}`}>{mark.sound}</span></button>
}
...
function NiqqudCard({ mark, closing = false, compact = false }: { mark: NiqqudMark; closing?: boolean; compact?: boolean }) {
  ...
  return <article ref={ref} className={`${compact ? 'mt-4' : ''} origin-top overflow-hidden rounded-[28px] border border-slate-200 bg-white text-left shadow-[0_16px_42px_rgba(15,23,42,0.08)]`}><div className="p-5"><div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-indigo-700">{mark.name}</p><p lang="he" dir="rtl" className="mt-1 break-words text-[1.3rem] font-black text-indigo-700">{mark.hebrewName}</p></div><div className="text-right"><p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Valor</p><p className="mt-1 text-lg font-black text-slate-500">—</p></div></div><div className="mt-4 grid grid-cols-2 items-center gap-4 border-y border-slate-100 py-5"><div className="text-center"><p lang="he" dir="rtl" className="text-[5.2rem] font-black leading-none text-indigo-700">{mark.visibleSign}</p><p className="mt-2 text-[11px] font-bold text-slate-400">Signo</p></div><div className="text-center"><p lang="he" dir="rtl" className="text-[2.6rem] font-black leading-tight text-slate-950">{mark.formula}</p><p className="mt-2 text-[1.05rem] font-black text-indigo-700">{mark.reading}</p></div></div><div className="mt-4 grid grid-cols-2 gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Sonido</p><p className="mt-1 text-[15px] font-black text-slate-900">{mark.sound}</p></div><div><p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Función</p><p className="mt-1 text-[14px] font-black leading-snug text-slate-900">{mark.functionLabel}</p></div></div><p className="mt-4 text-[14px] leading-relaxed text-slate-700">{mark.explanation}</p>{mark.caution && <p className="mt-3 rounded-[18px] bg-slate-50 px-4 py-3 text-[12px] font-semibold leading-relaxed text-slate-600">{mark.caution}</p>}</div></article>
}
...
function ListView({ marks }: { marks: readonly NiqqudMark[] }) {
  return <div className="overflow-hidden rounded-[22px] border border-slate-200 bg-white"><div className="grid grid-cols-[44px_minmax(65px,1fr)_42px_64px_minmax(82px,1.25fr)] gap-2 border-b border-slate-200 bg-slate-50 px-2.5 py-2 text-center text-[9px] font-black uppercase tracking-[0.06em] text-slate-400"><span>Signo</span><span>Nombre</span><span>Valor</span><span>Sonido</span><span>Función</span></div>{marks.map(mark => <div key={mark.id} className="grid min-h-[66px] grid-cols-[44px_minmax(65px,1fr)_42px_64px_minmax(82px,1.25fr)] items-center gap-2 border-b border-slate-100 px-2.5 py-2 text-center last:border-b-0"><span lang="he" dir="rtl" className="text-[2rem] font-black leading-none text-indigo-700">{mark.visibleSign}</span><span className="break-words text-[11px] font-black leading-tight text-slate-900">{mark.name}</span><span className="text-[12px] font-black text-slate-400">—</span><span className="break-words text-[11px] font-black leading-tight text-indigo-700">{mark.sound}</span><span className="break-words text-[10px] font-bold leading-tight text-slate-600">{mark.functionLabel}</span></div>)}</div>
}
