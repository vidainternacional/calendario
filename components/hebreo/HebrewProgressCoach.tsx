...
export default function HebrewProgressCoach() {
  ...
  useEffect(() => () => { try { recognitionRef.current?.stop() } catch { /* ya detenido */ }; void releaseMicrophone() }, [])
  ...
  const medals = useMemo(() => {
    ...
    return [
      award('Palabras', answers.filter(row => row.skill === 'vocabulary')),
      award('Lectura', answers.filter(row => row.skill === 'reading' && !row.question_key.startsWith('oral-'))),
      award('Pronunciación', answers.filter(row => row.question_key.startsWith('oral-'))),
      award('Reglas', answers.filter(row => row.skill === 'rules')),
    ]
  }, [answers])
  ...
  function oralSelectionFor(target: HebrewDifficulty, count: number) {
    const pool = ORAL_CHECKPOINTS[target]; const counts = new Map(pool.map(item => [item.key, answers.filter(answer => answer.question_key === item.key).length]))
    return [...pool].sort((a, b) => (counts.get(a.key) ?? 0) - (counts.get(b.key) ?? 0)).slice(0, Math.max(1, Math.min(count, pool.length)))
  }
  ...
  async function begin() {
    ...
    try {
      ...
      const areas = skillsForTraining(trainingFocus, focusAreas)
      const oralCount = oralCountFor(questionCount, trainingFocus)
      const writtenCount = trainingFocus === 'pronunciation' ? 0 : Math.max(1, questionCount - oralCount)
      ...
      const oralQuestions = oralCount > 0 ? oralSelectionFor(requested, oralCount) : []
      const sessionQuestions = [...normalQuestions, ...oralQuestions]
  ...
  async function advanceAfterSave() {
    await new Promise(resolve => window.setTimeout(resolve, 520)); setSpeechResult(null); setSpeechError(null); setVoiceStatus('idle'); capturedRef.current = ''; await releaseMicrophone()
  ...
  async function startListening() {
    ...
    try {
      await ensureMicrophone(); setVoiceStatus('listening'); await new Promise(resolve => window.setTimeout(resolve, 90)); const instance = new Recognition(); recognitionRef.current = instance; instance.lang = 'he-IL'; instance.interimResults = true; instance.continuous = false; instance.maxAlternatives = 1
      ...
      instance.onerror = event => { const code = event.error ?? ''; setSpeechError(code === 'not-allowed' ? 'El micrófono está bloqueado. Permite el acceso para este sitio y vuelve a intentarlo.' : code === 'no-speech' ? 'No detecté voz. Toca Hablar otra vez y espera a que el espectro se active.' : 'El reconocimiento se interrumpió. Toca Hablar nuevamente.') }
      instance.onend = () => { recognitionRef.current = null; pauseSpectrum(); const transcript = capturedRef.current.trim(); if (transcript) { setVoiceStatus('processing'); void submitPronunciation(transcript) } else { setVoiceStatus('idle'); void releaseMicrophone(); setSpeechError(previous => previous ?? 'No se reconoció texto. Puedes volver a tocar Hablar.') } }
      instance.start()
    } catch { recognitionRef.current = null; setVoiceStatus('idle'); await releaseMicrophone(); setSpeechError('El micrófono no pudo iniciar de forma estable. Toca Hablar nuevamente.') }
  }
  ...
  function stopListening() { try { recognitionRef.current?.stop() } catch { /* ya detenido */ } }
