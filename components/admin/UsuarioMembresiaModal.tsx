...
export default function UsuarioMembresiaModal({ usuario, todosMinisterios, isOpen, onClose }: { usuario: any | null; todosMinisterios: any[]; isOpen: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    if (!isOpen) { setError(''); setOverrides({}); setPuedeEliminar(false); setCurrentUserId(null); return }
    let cancelled = false
    void (async () => { const contexto = await obtenerContextoAdministrador(); if (!cancelled) { setPuedeEliminar(contexto.esAdministrador); setCurrentUserId(contexto.userId) } })()
    const previousOverflow = document.body.style.overflow
    document.body.style.overflow = 'hidden'
    const handleKeyDown = (event: KeyboardEvent) => { if (event.key === 'Escape' && !isProcessing) onClose() }
    window.addEventListener('keydown', handleKeyDown)
    return () => { cancelled = true; document.body.style.overflow = previousOverflow; window.removeEventListener('keydown', handleKeyDown) }
  ...
  const handleToggleMembresia = async (minId: string, isMember: boolean, currentlyLider: boolean) => {
    ...
    try {
      if (isMember && currentlyLider) { const liderazgo = await actualizarLiderazgoMinisterial(usuario.id, minId, false); if (!liderazgo.success) { setError(liderazgo.error || 'No fue posible quitar el liderazgo.'); return } }
      ...
      router.refresh()
    } catch (err: any) { setError(err.message || 'No fue posible actualizar la membresía.') } finally { setLoadingIds((p) => ({ ...p, [minId]: false })) }
  }
  ...
  const handleToggleLider = async (minId: string, currentlyLider: boolean) => {
    ...
    try {
      ...
      setOverrides((p) => ({ ...p, [minId]: { member: true, lider: !currentlyLider } })); router.refresh()
    } catch (err: any) { setError(err.message || 'No fue posible cambiar el liderazgo.') } finally { setLoadingIds((p) => ({ ...p, [`lider_${minId}`]: false })) }
  }
  ...
  const handleEliminarUsuario = async () => {
    ...
    try { const result = await eliminarUsuarioDefinitivamente(usuario.id); if (!result.success) { setError(result.error || 'No fue posible eliminar al usuario.'); return }; onClose(); router.refresh() }
    catch (err: any) { setError(err.message || 'No fue posible eliminar al usuario.') } finally { setDeleting(false) }
  }
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/45 p-3 backdrop-blur-sm sm:p-4" role="dialog" aria-modal="true" aria-labelledby="ficha-usuario-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !isProcessing) onClose() }}>
      <div className="flex max-h-[calc(100dvh-1.5rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] w-full max-w-md flex-col overflow-hidden rounded-[24px] bg-white shadow-2xl sm:max-h-[88vh]">
        <header className="flex shrink-0 items-center justify-between border-b border-slate-100 bg-white px-4 py-4 sm:px-5"><div className="min-w-0 pr-3"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-indigo-500">Administración</p><h3 id="ficha-usuario-title" className="mt-0.5 font-bold text-[#171923]">Ficha integral del miembro</h3></div><button type="button" onClick={onClose} disabled={isProcessing} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-slate-50 text-gray-500 disabled:opacity-50" aria-label="Cerrar ficha del usuario"><X className="h-5 w-5" /></button></header>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4 sm:px-5">
          ...
          {todosMinisterios.length === 0 ? <div className="flex min-h-48 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-5 text-center"><Users className="mb-3 h-8 w-8 text-slate-300" /><p className="text-sm font-semibold text-slate-600">No hay ministerios disponibles</p></div> : <div className="space-y-3">{todosMinisterios.map((ministerio) => {
            const { member, lider } = estadoMinisterio(ministerio.id); const loadingMem = loadingIds[ministerio.id]; const loadingLider = loadingIds[`lider_${ministerio.id}`]
