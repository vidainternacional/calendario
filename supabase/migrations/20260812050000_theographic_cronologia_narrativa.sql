...
-- - La propia documentación de Theographic marca Events/startDate/sortKey como Incomplete y
--   su bibliografía usa sistemas cronológicos editoriales concretos.
-- - VIDA conserva esos campos únicamente como provenance no visible.
-- - El orden de esta capa es narrativo/canónico: primer versículo enlazado por cada evento.
-- - Los títulos fuente están en inglés y permanecen staged hasta localización editorial.
...
begin
  ...
  left join pg_temp.vida_theographic_verses v
    on v.record_id=(verse_record.value #>> '{}')
  left join public.biblical_books b
    on b.canonical_order=v.book_order;
  ...
  select
    event_record_id,
    min(verse_id) as min_verse_id,
    max(verse_id) as max_verse_id,
  ...
  with ranked as (
    ...
    join public.biblical_books b_start
      on b_start.canonical_order=floor(s.min_verse_id/1000000.0)::integer
    join public.biblical_books b_end
      on b_end.canonical_order=floor(s.max_verse_id/1000000.0)::integer
  )
  insert into public.biblical_timeline_events(
    slug,title,summary,period_id,
    start_book_code,start_chapter,start_verse,end_book_code,end_chapter,end_verse,
  ...
  select
    'theographic-event-'||lower(r.record_id),
    r.fields->>'title',
    null,
    null,
    r.start_code,r.start_chapter::smallint,r.start_verse::smallint,
    r.end_code,r.end_chapter::smallint,r.end_verse::smallint,
    null,null,'relative',r.narrative_order,'relative','medium',null,
    v_source_id,
    v_events_url||'#'||r.record_id,
    'cfb1c485d4da6fb63a69cb3b7f5b0752792f46bc',
    ...
    jsonb_build_object(
      ...
      'source_chronology_not_for_display',jsonb_strip_nulls(jsonb_build_object(
        ...
        'rangeFlag',r.fields->'rangeFlag',
        'lag',r.fields->>'lag',
        'lagType',r.fields->>'lagType'
      )),
      'source_chronology_status','provider_documents event chronology fields as incomplete; retained only for provenance',
  ...
  insert into public.biblical_timeline_event_references(
    event_id,book_code,chapter,verse,source_id,source_locator,provider_version,
    content_hash,review_status,enabled,metadata
  )
  select
    event.id,
    ref.book_code,ref.chapter::smallint,ref.verse::smallint,
    v_source_id,
  ...
  join public.biblical_timeline_events event
    on event.slug='theographic-event-'||lower(ref.event_record_id)
