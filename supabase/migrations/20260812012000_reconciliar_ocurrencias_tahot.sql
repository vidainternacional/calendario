...
begin
  ...
  -- El corpus completo ya contiene las mismas unidades en word_index 2–4.
  if (
    select count(*)
  ...
  select
    ...
    jsonb_build_object(
      ...
      'source_line',138269,
      'source_lemma','יְמִינִי',
      'source_gloss_en','jaminite[s]',
      'post_punctuation_lexical_recovery',true,
      'punctuation_tag','H9015',
      'punctuation_surface','׀',
      'generated_by_ai',false,
      'direct_import_version','tahot-core-v5.1'
    ),
    6,3,'word','1ch-27-12-06','ye.mi.ni',null,'׀',null,false,false,
    'base',null,
  ...
  do update set
    ...
    display_word_index=excluded.display_word_index,
    token_kind=excluded.token_kind,
    word_group_key=excluded.word_group_key,
    occurrence_transliteration=excluded.occurrence_transliteration,
    punctuation_before=excluded.punctuation_before,
    joins_previous=excluded.joins_previous,
    joins_next=excluded.joins_next,
    textual_status=excluded.textual_status,
  ...
  select
    ...
    jsonb_build_object(
      ...
      'source_line',170046,
      'source_lemma','פָּרַץ',
      'source_gloss_en','[were] broken down',
      'post_punctuation_lexical_recovery',true,
      'punctuation_tag','H9015',
      'punctuation_surface','׀',
      'generated_by_ai',false,
    ...
    17,2,'word','neh-2-13-17','fe.ru.tzim',null,'׀',null,false,false,
    'base',null,
    jsonb_build_object('text_suffix','=Q(K)','source_textual_status','qere','language','hebrew')
  from public.biblical_lexical_entries e
  ...
  if v_inserted <> 2 then
    raise exception 'Se esperaban 2 raíces Qere reconciliadas, se procesaron %',v_inserted;
  end if;

  update internal.biblical_textual_import_batches b
  set metadata=b.metadata||jsonb_build_object(
        'reconciliation','tahot-post-punctuation-v1',
        'post_punctuation_lexical_recoveries',1
      ),
      updated_at=now()
  where b.source_id=v_source_id
    and b.source_commit='b86d26cdb1f51729e73b5b4eb7f7ccadc5dfba39'
    and b.book_code in ('1CH','NEH');

  update internal.biblical_textual_import_batches b
  set metadata=b.metadata||jsonb_build_object(
        'reconciliation','psa23-pilot-cleanup-v1',
        'legacy_occurrences_removed',3
      ),
      updated_at=now()
  where b.source_id=v_source_id
    and b.source_commit='b86d26cdb1f51729e73b5b4eb7f7ccadc5dfba39'
