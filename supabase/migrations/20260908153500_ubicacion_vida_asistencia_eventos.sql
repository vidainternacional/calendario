...
create or replace function public.asistencia_evento_visible_para(
  p_evento_id uuid,
  p_profile_id uuid
)
returns boolean
...
as $$
  with event_calendars as (
    select e.calendar_id as calendar_id
...
create or replace function public.asistencia_persona_esperada(
  p_evento_id uuid,
  p_profile_id uuid
)
returns boolean
...
create or replace function public.asistencia_racha_sin_confirmar(p_profile_id uuid)
returns integer
language sql
stable
security definer
  ...
  with recientes as (
    ...
    select
      ...
      sum(case when estado is not null then 1 else 0 end)
        over (order by fecha_inicio desc rows between unbounded preceding and current row) as corte
    from recientes
  )
...
create policy evento_asistencia_config_select
  ...
  using (
    ...
    or exists (
      select 1 from public.eventos e where e.id = evento_id
    )
  );
...
create policy evento_asistencias_select
  ...
  using (
    profile_id = auth.uid()
    or public.puede_gestionar_asistencia()
  );
...
as $$
begin
  if not public.puede_gestionar_asistencia() then
...
begin
  ...
  if not public.asistencia_evento_visible_para(p_evento_id, auth.uid()) then
    raise exception 'Evento no disponible';
  end if;
  ...
  if v_cfg.evento_id is null then
    raise exception 'Este evento no tiene confirmación de asistencia activa';
  end if;

  select e.fecha_inicio into v_inicio
  from public.eventos e
  where e.id = p_evento_id;
  ...
  if v_distancia > v_cfg.radio_metros then
    raise exception 'Aún no estás dentro del área del evento';
  end if;
  ...
  on conflict (evento_id, profile_id)
  do update set
    estado = 'asistio',
    confirmado_en = now(),
    metodo = 'gps';
...
create or replace function public.asistencia_justificar(
  p_evento_id uuid,
  p_profile_id uuid
)
returns void
...
begin
  ...
  if exists (
    select 1
    from public.evento_asistencias a
...
begin
  ...
  if not exists (
    select 1 from public.evento_asistencia_config c
  ...
  select
    ...
    a.confirmado_en,
    a.metodo,
    public.asistencia_racha_sin_confirmar(p.id)
  from public.profiles p
  left join public.evento_asistencias a
    on a.evento_id = p_evento_id
   and a.profile_id = p.id
  where p.activo = true
    and p.estado_cuenta = 'activo'
    and public.asistencia_persona_esperada(p_evento_id, p.id)
  order by
    case coalesce(a.estado, 'sin_confirmacion')
      when 'sin_confirmacion' then 0
      when 'justificado' then 1
      else 2
    end,
    p.nombre_completo;
end;
...
begin
  ...
  if p_profile_id <> auth.uid() and not public.puede_gestionar_asistencia() then
    raise exception 'Sin permiso';
  end if;
...
revoke execute on function public.asistencia_racha_sin_confirmar(uuid) from public, anon, authenticated;
