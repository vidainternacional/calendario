-- FASE H / Bloque 3 — corrección del lote de nombres 002.
-- Hallazgo: el gate anterior podía enlazar una entrada cuyo label izquierdo no
-- coincidía con la entidad TIPNR a la derecha de `»` (ej.: Anakim»Anak).
-- Esta migración elimina cualquier salida previa del batch 002 y vuelve a
-- insertar únicamente entidades exactas, con doble evidencia española.
-- Reversión de la nueva carga exacta:
-- DELETE FROM public.biblical_hebrew_spanish_glosses
-- WHERE provenance->>'batch_id' = 'fase_h_es_nombres_wikidata_anchor2_exact_002_20260820';
...
with map(tipnr_id, anchor_ref, wikidata_id, english_label, display_gloss_es, source_uri, source_revision) as (
 ...
 select map.*, count(distinct v.source_id) as spanish_anchor_sources
 from map
 ...
 select e.id lexical_entry_id,e.strong_number,e.source_gloss,evidence.*
 ...
 where e.language='hebrew' and e.review_status='approved' and e.enabled=true
...
select lexical_entry_id,display_gloss_es,'{}'::text[],99,'tipnr_wikidata_spanish_anchor_2source_exact_entity_v2',source_gloss,'verified_derived',
 jsonb_build_object('phase','FASE_H_BLOQUE_3','batch_id','fase_h_es_nombres_wikidata_anchor2_exact_002_20260820','source_identity','STEPBible TIPNR','source_identity_license','CC BY 4.0','wikidata_id',wikidata_id,'wikidata_uri',source_uri,'wikidata_license','CC0-1.0','wikidata_revision',source_revision,'english_identity_label',english_label,'anchor_reference',anchor_ref,'spanish_anchor_sources',spanish_anchor_sources,'spanish_anchor_sources_minimum',2,'exact_source_entity',true,'anchor_used_for_name_spelling_only',true,'context_used_as_meaning',false,'rv1909_used_as_meaning',false,'strong_number',strong_number)
from eligible
