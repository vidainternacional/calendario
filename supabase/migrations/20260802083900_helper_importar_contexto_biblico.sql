-- FASE D · Bloque 4
-- Importador idempotente para lotes editoriales de contexto bíblico.
...
begin
  ...
  for b in select value from jsonb_array_elements(p_books) loop
    v_book_code := b->>'code';
    ...
    insert into public.biblical_context_units(
      ...
      v_slug, v_book_code, v_source_id, 'book', 1, null,
      (b->>'chapters')::smallint, null,
      (b->>'name') || ': perfil histórico y literario',
      b->>'summary', b->>'historical', b->>'jewish', b->>'literary',
      b->>'intent', b->>'reflection', b->>'cautions',
      ...
      'vida://corpus/contexto/' || p_batch || '/v1#' || v_slug,
      p_version,
      encode(extensions.digest(concat_ws('|', v_slug, b->>'summary', b->>'historical',
        b->>'jewish', b->>'literary', b->>'intent', b->>'reflection', b->>'cautions'), 'sha256'), 'hex'),
      'approved', true,
      jsonb_build_object(
        'coverage_batch', p_batch,
        'generated_by_ai', true,
        'review_level', 'ai_assisted_editorial',
        'human_review_status', 'pending',
        'canonical_seed', true
    ...
    set metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
          'coverage_status', 'context_ready',
          'coverage_batch', p_batch,
          'coverage_version', p_version
        ),
        updated_at = now()
  ...
  for s in select value from jsonb_array_elements(p_sections) loop
    select name_es into v_book_name
    ...
    v_slug := coalesce(
      nullif(s->>'slug', ''),
      lower(regexp_replace(v_book_name, '[^a-zA-Z0-9]+', '-', 'g')) || '-' || (s->>'start') || '-' || (s->>'end')
    );
    ...
    insert into public.biblical_context_units(
      ...
      v_slug, s->>'code', v_source_id, 'section',
      (s->>'start')::smallint, null, (s->>'end')::smallint, null,
      s->>'title', s->>'summary',
      'Esta unidad se desarrolla dentro del marco histórico general de ' || v_book_name ||
        ' y concentra los capítulos ' || (s->>'start') || '–' || (s->>'end') || '. ' || v_historical,
      ...
      s->>'intent',
      coalesce(nullif(s->>'reflection', ''), (
        select theological_reflection
        from public.biblical_context_units
        where book_code = s->>'code'
          and scope_kind = 'book'
          and enabled
      ...
      'vida://corpus/contexto/' || p_batch || '/v1#' || v_slug,
      p_version,
      encode(extensions.digest(concat_ws('|', v_slug, s->>'title', s->>'summary',
        v_historical, v_jewish, v_literary, s->>'intent', s->>'reflection', s->>'cautions'), 'sha256'), 'hex'),
      'approved', true,
      jsonb_build_object(
        'coverage_batch', p_batch,
        'generated_by_ai', true,
