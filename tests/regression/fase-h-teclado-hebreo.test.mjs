...
test('FASE H teclado: se integra globalmente sin alterar la guardia de sesión', () => {
  assert.match(page, /import HebrewKeyboardDock/)
  assert.match(page, /<HebrewLearningHome \/>/)
  assert.match(page, /<HebrewKeyboardDock \/>/)
...
test('FASE H teclado: conserva letras hebreas formas finales y niqqud', () => {
  for (const letter of ['א', 'ב', 'ג', 'ד', 'ה', 'ו', 'ז', 'ח', 'ט', 'י', 'כ', 'ל', 'מ', 'נ', 'ס', 'ע', 'פ', 'צ', 'ק', 'ר', 'ש', 'ת']) {
    assert.match(keyboard, new RegExp(letter))
  }
  for (const finalForm of ['ך', 'ם', 'ן', 'ף', 'ץ']) assert.match(keyboard, new RegExp(finalForm))
  for (const name of ['Sheva', 'Hataf Segol', 'Hataf Pataj', 'Hataf Qamats', 'Hiriq', 'Tsere', 'Segol', 'Pataj', 'Qamats', 'Holam', 'Qubuts', 'Dagesh']) {
    assert.match(keyboard, new RegExp(name))
  }
})
