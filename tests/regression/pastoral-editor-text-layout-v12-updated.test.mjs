...
test('Fondos cambian solo la apariencia de la página y no remaquetan texto existente', () => {
  ...
  assert.doesNotMatch(fondo, /elementos|tamano_fuente|contenido|textoMuestraPlantilla|layout\.pt|layout\.x|layout\.y|layout\.w|layout\.h|fuente:|alineacion:/)
  assert.doesNotMatch(paleta, /fuente:/)
})

test('Fondos crecen verticalmente en filas de tres sin reintroducir Temas', () => {
  const fondos = workspace.slice(workspace.indexOf("panel === 'plantillas'"), workspace.indexOf("panel === 'temas'"))
  assert.match(fondos, /grid grid-cols-3 gap-x-2 gap-y-3/)
  assert.match(fondos, /aria-label="Fondos en filas de tres"/)
  assert.match(fondos, /PLANTILLAS_VISUALES\.map/)
  assert.doesNotMatch(fondos, /PALETAS_PRESENTACION\.map|aria-label="Temas en filas de tres"|pastoral-template-grid|overflow-x-auto/)
})
