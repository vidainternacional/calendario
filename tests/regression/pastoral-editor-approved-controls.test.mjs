...
test('editor conserva retorno, fondo editable y presentación real separada de Compartir', () => {
  ...
  assert.match(presentar, /onClick=\{abrirPantallaCompleta\}/)
  assert.match(presentar, /onClick=\{cerrarPantallaCompleta\}/)
  assert.match(presentar, /moverPresentacion\(/)
  assert.match(presentar, /<PastoralVisualCanvas pagina=\{paginaPresentacion\} biblioteca=\{biblioteca\} fitViewport=\{modoPresentacion\} \/>/)
  assert.match(presentar, /<MaterialPastoralExperience material=\{\{ \.\.\.paquete, presentacion_diapositivas: paginas, coleccion \}\} biblioteca=\{biblioteca\} embeddedStudy \/>/)
