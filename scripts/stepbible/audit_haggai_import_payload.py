#!/usr/bin/env python3
"""Audita el payload determinista de Hageo sin modificar Supabase."""
...
EXPECTED_PACKAGE_SHA = "bc8e1caebce9a2e55d34b3be4770f3591e430b3aa217208324dee1bdbdd54e38"
...
def audit(payload: dict[str, Any], file_sha256: str) -> dict[str, Any]:
    ...
    if payload.get("book") != {
        ...
        raise RuntimeError(f"Identidad de Hageo inesperada: {payload.get('book')}")
    if payload.get("package_sha256") != EXPECTED_PACKAGE_SHA:
        raise RuntimeError("El payload no deriva del paquete fijado de Hageo")
    ...
    for verse in payload["verse_texts"]:
        ...
        if indexes != expected:
            raise RuntimeError(
                f"Índices visibles discontinuos en Hageo {key[0]}:{key[1]}"
            )
    ...
    if actual_variants != expected_variants:
        raise RuntimeError(
            "Variantes estructuradas inesperadas: "
            f"actual={sorted(actual_variants, key=str)}"
        )
    if len({row["variant_key"] for row in payload["variants"]}) != 3:
        raise RuntimeError("Claves de variantes duplicadas")
    ...
    return {
        "schema_version": "vida-haggai-import-payload-audit-v1",
        "book": payload["book"],
...
def self_test() -> None:
    ...
    print("Auto-test de auditoría de payload de Hageo: OK")
