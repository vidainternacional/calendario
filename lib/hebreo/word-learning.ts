...
export const HEBREW_LEARNING_GROUPS: readonly HebrewLearningGroup[] = [
  { id: 'essentials', label: 'Esenciales', description: 'Palabras frecuentes y fáciles de reconocer para comenzar a leer y recordar vocabulario.' },
  { id: 'family', label: 'Familia', description: 'Personas y relaciones familiares básicas.' },
  { id: 'daily', label: 'Vida diaria', description: 'Casa, comida, agua, tiempo y palabras de uso cotidiano.' },
  ...
  { id: 'faith', label: 'Fe y conceptos', description: 'Palabras centrales para leer textos bíblicos: Dios, paz, verdad, instrucción y santidad.' },
  { id: 'actions', label: 'Acciones', description: 'Verbos básicos para reconocer lo que alguien dice, hace, ve, oye o realiza.' },
  { id: 'nouns', label: 'Sustantivos', description: 'Explora las palabras clasificadas como sustantivos en el léxico aprobado.' },
  { id: 'verbs', label: 'Verbos', description: 'Explora las palabras clasificadas como verbos en el léxico aprobado.' },
  { id: 'adjectives', label: 'Adjetivos', description: 'Explora las palabras clasificadas como adjetivos en el léxico aprobado.' },
  { id: 'all', label: 'Todas', description: 'Acceso al catálogo hebreo aprobado completo, paginado para mantener la app ligera.' },
] as const
...
type PedagogicalWord = {
  ...
  topics: readonly Exclude<HebrewLearningGroupId, 'nouns' | 'verbs' | 'adjectives' | 'all'>[]
  meaning?: string
}
...
export function pedagogicalWordForId(lexicalId: string) {
  return PEDAGOGICAL_BY_ID.get(lexicalId) ?? null
}

export function lexicalIdsForSpanishSearch(search: string) {
  const normalized = normalizeSpanish(search)
  if (!normalized) return []
  return PEDAGOGICAL_HEBREW_WORDS
    .filter(word => {
      const spanish = normalizeSpanish(word.spanish)
      const meaning = normalizeSpanish(word.meaning ?? '')
      return spanish.includes(normalized) || meaning.includes(normalized)
    })
    .map(word => word.lexicalId)
}
