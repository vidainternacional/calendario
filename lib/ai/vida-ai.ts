...
function configuredProviderOrder(level: VidaAiLevel): VidaAiProviderName[] {
  const raw = level === 1
    ? process.env.VIDA_AI_ECONOMY_ORDER || 'gemini,openai,claude,grok,kimi,perplexity'
    : process.env.VIDA_AI_ADVANCED_ORDER || 'openai,claude,grok,gemini,kimi,perplexity'
...
function readCache(key: string) {
  ...
  if (entry.expiresAt <= Date.now()) { privateCache.delete(key); return null }
  return { ...entry.result, cached: true }
}
function writeCache(key: string, result: VidaAiResult, ttlMs: number) {
  if (privateCache.size >= MAX_CACHE_ENTRIES) { const oldestKey = privateCache.keys().next().value as string | undefined; if (oldestKey) privateCache.delete(oldestKey) }
...
function enforceBurstLimit(ownerId: string, task: VidaAiTask) {
  const key = ownerTaskKey(ownerId, task); const now = Date.now(); const window = burstWindows.get(key)
  if (!window || now - window.startedAt >= BURST_WINDOW_MS) { burstWindows.set(key, { startedAt: now, count: 1 }); return }
  if (window.count >= BURST_MAX_REQUESTS) throw new VidaAiError('rate_limited', 'Se alcanzó el límite temporal de solicitudes para esta tarea.')
  window.count += 1
}
...
function providerAvailable(provider: VidaAiProviderName, model: string) {
  const key = cooldownKey(provider, model); const until = providerCooldowns.get(key) ?? 0
  if (until <= Date.now()) { providerCooldowns.delete(key); return true }
  return false
}
...
async function callClaude(model: string, apiKey: string, request: VidaAiRequest, policy: VidaAiPolicy): Promise<ProviderResult> {
  const response = await fetchWithTimeout('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model, system: request.instructions, messages: [{ role: 'user', content: request.input }], max_tokens: policy.maxOutputTokens }),
  }, policy.timeoutMs)
  if (!response.ok) throw new ProviderError(`Claude HTTP ${response.status}`, response.status)
...
function openAiCompatibleEndpoint(provider: 'grok' | 'perplexity' | 'kimi') {
...
async function callProvider(provider: VidaAiProviderName, model: string, apiKey: string, request: VidaAiRequest, policy: VidaAiPolicy) {
  ...
  if (provider === 'claude') return callClaude(model, apiKey, request, policy)
