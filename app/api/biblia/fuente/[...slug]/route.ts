...
async function resolverBibleId(virtualId: string) {
  ...
  if (virtualId === 'api-nvi') return bibles.find(esNvi)?.id || null
  if (virtualId === 'api-rvr1960') return bibles.find(esRvr1960)?.id || null
...
function normalizarTraduccionesApiBible(items: any[]) {
  return [...items].sort(ordenarApiBible).map((item: any) => ({
    id: idVirtual(item),
...
function extraerVersos(content: unknown, chapterId: string) {
  ...
  const caminar = (node: any, numeroActual?: number) => {
    ...
    if (actual && typeof node.text === 'string') {
      const limpio = node.text.replace(/\s+/g, ' ')
      if (limpio.trim()) porNumero.set(actual, `${porNumero.get(actual) || ''}${limpio}`.trim())
    }
  ...
  return [...porNumero.entries()]
    .sort((a, b) => a[0] - b[0])
    .map(([number, text]) => ({ type: 'verse', number, content: [{ text: text.replace(/\s+/g, ' ').trim() }] }))
}
