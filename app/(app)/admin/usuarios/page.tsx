...
export default async function AdminUsuariosPage() {
  ...
  const [{ data: usuarios }, { data: ministerios }, { data: pushRows }, { data: activityRows }, { data: detailRows }] = await Promise.all([
  ...
  return (
    <main className="mx-auto min-h-screen max-w-2xl bg-[#f4f5f9] px-4 pb-[calc(7rem+env(safe-area-inset-bottom))] pt-[calc(env(safe-area-inset-top)+4.75rem)] sm:px-6 sm:pt-12">
      ...
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-500">Consulta la ficha integral de cada miembro: identidad, actividad, ministerios, profesión, disponibilidad, habilidades y permisos.</p>
      </header>
