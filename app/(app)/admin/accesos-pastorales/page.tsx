...
export default async function AccesosPastoralesPage() {
  ...
  const { data: usuarios, error } = await (supabase as any)
    .from('profiles')
    .select('id, nombre_completo, email, rol, estado_cuenta, acceso_centro_pastoral')
  ...
  return (
    <main className="mx-auto min-h-screen max-w-2xl bg-[#f4f5f9] px-4 pb-[calc(8rem+env(safe-area-inset-bottom))] pt-[calc(1.5rem+env(safe-area-inset-top))] sm:px-6 sm:pt-8">
      <Link href="/admin" className="inline-flex min-h-11 items-center gap-2 rounded-xl text-sm font-bold text-indigo-700">
      ...
      <header className="mb-6 mt-3">
        <div className="flex items-center gap-2 text-indigo-600">
          ...
          <p className="text-xs font-bold uppercase tracking-[0.16em]">Permisos especiales</p>
        </div>
        <h1 className="mt-2 text-2xl font-bold text-slate-950 sm:text-3xl">Acceso al Centro Pastoral</h1>
        <p className="mt-1.5 text-sm leading-6 text-slate-500">
          Autoriza a líderes u otras personas para preparar sus propios estudios sin cambiar su rol dentro de la aplicación.
        </p>
      </header>
      ...
      <section className="mb-5 flex items-start gap-3 rounded-[20px] border border-indigo-100 bg-indigo-50 p-4">
        ...
        <div>
          <h2 className="text-sm font-bold text-indigo-950">Contenido privado por persona</h2>
          <p className="mt-1 text-xs leading-5 text-indigo-800/80">Cada usuario autorizado verá únicamente sus propios bosquejos, colecciones, archivos y paquetes. Los pastores y administradores mantienen acceso automático por su rol.</p>
        </div>
      </section>
