...
import {
  BarChart3, BookOpenCheck, Building2, CheckCircle2, ChevronRight,
...
export default async function AdminPage() {
  ...
  const [
    ...
    (supabase as any).from('profiles').select('*', { count: 'exact', head: true }).eq('estado_cuenta', 'pendiente'),
  ])
  ...
  const totalPendientes = (pendingIngresos || 0) + (pendingAvisos || 0) + (pendingPreguntas || 0) + (pendingUsuarios || 0)
  const modulos = [
  ...
  return (
    <main className="mx-auto min-h-screen max-w-2xl bg-[#f4f5f9] px-4 pb-[calc(7rem+env(safe-area-inset-bottom))] pt-[calc(env(safe-area-inset-top)+4.75rem)] sm:px-6 sm:pt-12">
      ...
      <section className="grid grid-cols-2 gap-3">
        {modulos.map(({ label, value, detail, href, icon: Icon, tone }) => <Link key={label} href={href} className="rounded-[20px] bg-white p-4 shadow-sm ring-1 ring-black/[0.04] active:scale-[0.99]"><div className="flex items-start justify-between gap-2"><span className={`relative flex h-9 w-9 items-center justify-center rounded-xl ${tone}`}><Icon className="h-4.5 w-4.5" />{href === '/admin/usuarios' && (pendingUsuarios || 0) > 0 && <span className="absolute -right-2 -top-2 grid h-5 min-w-5 place-items-center rounded-full bg-rose-500 px-1 text-[9px] font-black leading-none text-white ring-2 ring-white">{(pendingUsuarios || 0) > 99 ? '99+' : pendingUsuarios}</span>}</span><ChevronRight className="h-4 w-4 text-slate-300" /></div><p className="mt-3 text-2xl font-extrabold text-[#171923]">{value}</p><p className="mt-1 text-xs font-bold text-slate-700">{label}</p><p className="mt-0.5 text-[10px] leading-4 text-slate-400">{detail}</p></Link>)}
      </section>
      ...
      <section className="mt-5 space-y-3">
        <Link href="/admin/analisis/estudio" className="flex items-center gap-3 rounded-[20px] bg-white p-4 shadow-sm ring-1 ring-black/[0.04]"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-50 text-violet-600"><BarChart3 className="h-5 w-5" /></span><div className="min-w-0 flex-1"><p className="text-sm font-bold text-[#171923]">Analíticas de Estudio</p><p className="text-[11px] text-slate-400">Pasajes, libros, temas, recurrencia y uso</p></div><ChevronRight className="h-4 w-4 text-slate-300" /></Link>
