...
export default async function AdminPage() {
  ...
  const [
    { count: usuarios },
    { count: ministerios },
    { count: pendingPreguntas },
    { count: pendingAvisos },
    { count: pendingIngresos },
  ] = await Promise.all([
  ...
  return (
    <main className="mx-auto min-h-screen max-w-2xl bg-[#f4f5f9] px-4 pb-[calc(7rem+env(safe-area-inset-bottom))] pt-[calc(env(safe-area-inset-top)+1.5rem)] sm:px-6 sm:pt-8">
      ...
      <header className="mb-5">
        ...
        <p className="mt-1.5 text-sm leading-6 text-slate-500">Entra directamente a cada área. El panel principal ya no mezcla las herramientas entre sí.</p>
      </header>

      {totalPendientes === 0 && (usuarios || 0) > 0 && (
      ...
      <section aria-label="Módulos administrativos" className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {modulos.map(({ label, value, detail, href, icon: Icon, tone }) => (
          <Link key={label} href={href} className="group min-w-0 rounded-[20px] border border-slate-100 bg-white p-4 shadow-sm transition-all active:scale-[0.98]">
            <div className="flex items-start justify-between gap-2">
              <span className={`flex h-10 w-10 items-center justify-center rounded-xl ${tone}`}><Icon className="h-4.5 w-4.5" /></span>
            ...
            <p className="mt-0.5 line-clamp-2 text-[10px] leading-4 text-slate-400">{detail}</p>
          </Link>
        ))}
      </section>

      <section className="mt-6" aria-labelledby="herramientas-admin">
        <h2 id="herramientas-admin" className="mb-3 px-1 text-sm font-bold text-[#171923]">Herramientas especializadas</h2>
        <div className="space-y-3">
          {currentUserRol === 'administrador' && (
            <Link href="/admin/accesos-pastorales" className="flex items-center gap-3 rounded-[18px] border border-slate-100 bg-white p-4 shadow-sm active:scale-[0.99]">
              ...
              <div className="min-w-0 flex-1"><p className="text-sm font-bold text-[#171923]">Accesos pastorales</p><p className="mt-0.5 text-[11px] text-slate-500">Permisos para preparar estudios y materiales.</p></div><ChevronRight className="h-4 w-4 text-slate-300" />
            </Link>
          )}
          <Link href="/admin/ayuda-solidaria" className="flex items-center gap-3 rounded-[18px] border border-slate-100 bg-white p-4 shadow-sm active:scale-[0.99]">
            ...
            <div className="min-w-0 flex-1"><p className="text-sm font-bold text-[#171923]">Ayuda Solidaria</p><p className="mt-0.5 text-[11px] text-slate-500">Solicitudes y aportes comunitarios.</p></div><ChevronRight className="h-4 w-4 text-slate-300" />
          </Link>
          <Link href="/admin/configuracion" className="flex items-center gap-3 rounded-[18px] border border-slate-100 bg-white p-4 shadow-sm active:scale-[0.99]">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-600"><Settings2 className="h-5 w-5" /></span>
            <div className="min-w-0 flex-1"><p className="text-sm font-bold text-[#171923]">Configuración avanzada</p><p className="mt-0.5 text-[11px] text-slate-500">Ícono de la app y ajustes de Estudio Profundo.</p></div><ChevronRight className="h-4 w-4 text-slate-300" />
          </Link>
        </div>
      </section>
    </main>
