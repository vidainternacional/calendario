...
export default async function AdminConfiguracionPage() {
  ...
  if (rol === 'administrador') {
    ...
    const { data: templateSetting } = await (admin as any).from('app_settings').select('valor').eq('clave', 'pastoral_templates').maybeSingle()
  ...
  return (
    <main className="mx-auto min-h-screen max-w-2xl bg-[#f4f5f9] px-4 pb-[calc(7rem+env(safe-area-inset-bottom))] pt-[calc(env(safe-area-inset-top)+4.75rem)] sm:px-6 sm:pt-12">
      ...
      <ConfiguracionAdminClient activeIconVariant={activeIconVariant} initialEstudioPrompt={estudioPrompt} isAdministrator={rol === 'administrador'} initialPastoralTemplates={pastoralTemplates} />
    </main>
  )
}
