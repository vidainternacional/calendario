...
export const metadata: Metadata = { title: 'Traductor | Hebreo Bíblico' }

export default async function HebrewTranslatorPage() {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect('/login?next=/estudios/hebreo/traductor')
