...
export default async function PerfilPage() {
  ...
  const roles = {
    servidor: { bg: 'bg-slate-500/10', text: 'text-slate-400', border: 'border-slate-500/20', label: 'Servidor' },
      ...
      <div className="space-y-8">
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">{(profile as any)?.nombre_completo}</h2>
        ...
        {/* Ministerios y Roles */}
        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <h3 className="text-lg font-semibold text-white mb-4">Tus Ministerios</h3>
          
          {!membresias || membresias.length === 0 ? (
            <p className="text-sm text-slate-500">No perteneces a ningún ministerio aún.</p>
          ) : (
            <div className="space-y-4">
              {membresias.map((m: any) => (
                <div key={m.id} className="flex items-center justify-between pb-4 border-b border-slate-800/50 last:border-0 last:pb-0">
                  <div className="flex items-center gap-3">
                    <div 
                      className="w-3 h-3 rounded-full"
                      style={{ backgroundColor: m.ministerios.color_primario }}
                    />
                    <span className="font-medium text-slate-200">{m.ministerios.nombre}</span>
                  </div>
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          ...
          {!membresias || membresias.length === 0 ? (
            <p className="text-sm text-slate-500">No hay ministerios para configurar.</p>
          ) : (
            <div className="space-y-4">
              {membresias.map((m: any) => (
