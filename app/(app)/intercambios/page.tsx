...
function estadoNormalizado(estados: string[]): EstadoAsignacionMusico {
...
export default async function MisServiciosPage() {
  ...
  const ministerioIdsAsignacion = Array.from(new Set((asignacionesRows as any[]).map((row) => String(row.ministerio_id || '')).filter(Boolean)))

  const [eventosReq, capacidadesReq] = await Promise.all([
    eventoIds.length
      ? admin.from('eventos').select('id,titulo,fecha_inicio,ubicacion,ministerio_id').in('id', eventoIds)
      : Promise.resolve({ data: [] }),
    capacidadIds.length
      ? admin.from('ministerio_capacidades').select('id,nombre,ministerio_id').in('id', capacidadIds)
      : Promise.resolve({ data: [] }),
  ])
  ...
  for (const row of asignacionesRows as any[]) {
    const evento = eventosMap.get(String(row.evento_id))
    if (!evento || new Date(evento.fecha_inicio).getTime() < ahora) continue

    const capacidad = row.capacidad_id ? capacidadesMap.get(String(row.capacidad_id)) : null
    const ministerioId = String(row.ministerio_id || capacidad?.ministerio_id || evento.ministerio_id || '')
    if (!ministerioId) continue
    const ministerio = ministeriosMap.get(ministerioId)
    ...
    actual.estado = estadoNormalizado(actual.estados)
    serviciosMap.set(key, actual)
  }

  const servicios = Array.from(serviciosMap.values()).sort(
  ...
  if (proximo) {
    const [repertorioReq, paletaReq] = await Promise.all([
      admin
        ...
        .select('id,orden,titulo,tonalidad,enlace,notas,spotify_url,youtube_url,cancion_id')
        .eq('evento_id', proximo.eventoId)
        .eq('ministerio_id', proximo.ministerioId)
        .order('orden')
        .order('created_at'),
      admin
        .from('evento_paletas')
        .select('colores,observaciones,referencia_url,ministerio_id,updated_at')
        .eq('evento_id', proximo.eventoId)
        .or(`ministerio_id.eq.${proximo.ministerioId},ministerio_id.is.null`)
    ...
    if (cancionIds.length) {
      const { data: canciones = [] } = await admin
  ...
  return (
    <main className="mx-auto min-h-screen max-w-lg bg-[#f4f5f9] px-4 pb-[calc(7rem+env(safe-area-inset-bottom))] pt-[calc(1.5rem+env(safe-area-inset-top))] sm:pt-8">
        ...
        <section className="overflow-hidden rounded-[26px] bg-white shadow-[0_8px_30px_rgba(20,24,40,0.08)] ring-1 ring-black/[0.04]">
          <div className="p-5" style={{ background: `linear-gradient(145deg, ${proximo.color}16, #ffffff 68%)` }}>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {proximo.funciones.map((funcion) => <span key={funcion} className="rounded-full bg-slate-900 px-3 py-1.5 text-[10px] font-extrabold text-white">{funcion}</span>)}
            </div>

            <div className="mt-5 border-t border-slate-100 pt-4">
              <p className="mb-2 text-[10px] font-extrabold uppercase tracking-[0.13em] text-slate-400">Tu respuesta</p>
              <EstadoAsignacionMusico ministerioId={proximo.ministerioId} eventoId={proximo.eventoId} initialEstado={proximo.estado} />
            </div>
          </div>
          ...
          <div className="border-t-[6px] border-slate-100 p-4">
            ...
            {repertorio.length === 0 ? (
              <p className="mt-2 text-xs text-slate-400">Todavía no hay canciones publicadas para este servicio.</p>
            ) : (
              <div className="mt-3 space-y-2">
                {repertorio.map((row: any, index: number) => {
                  const titulo = row.cancion?.titulo || row.titulo || 'Canción'
                  const artista = row.cancion?.artista || null
                  const spotify = row.cancion?.spotify_url || row.spotify_url || null
                  const youtube = row.cancion?.youtube_url || row.youtube_url || null
                  return (
                    <div key={row.id} className="rounded-2xl bg-violet-50/60 p-3 ring-1 ring-violet-100/70">
                      <div className="flex items-start gap-2.5">
                        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-white text-[10px] font-extrabold text-violet-600 ring-1 ring-violet-100">{index + 1}</span>
                        <div className="min-w-0 flex-1">
                          <div className="flex min-w-0 items-center gap-2"><p className="truncate text-xs font-extrabold text-slate-800">{titulo}</p>{row.tonalidad && <span className="shrink-0 rounded-full bg-white px-2 py-1 text-[9px] font-extrabold text-slate-500">{row.tonalidad}</span>}</div>
                          {artista && <p className="mt-0.5 truncate text-[10px] text-slate-400">{artista}</p>}
                          {(spotify || youtube || row.enlace) && <div className="mt-2 flex flex-wrap gap-1.5">{spotify && <a href={spotify} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg bg-white px-2.5 py-1.5 text-[9px] font-bold text-slate-600">Spotify <ExternalLink className="h-3 w-3" /></a>}{youtube && <a href={youtube} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg bg-white px-2.5 py-1.5 text-[9px] font-bold text-slate-600">YouTube <ExternalLink className="h-3 w-3" /></a>}</div>}
                          {row.notas && <p className="mt-2 text-[10px] leading-4 text-slate-500">{row.notas}</p>}
                        </div>
                      </div>
                    </div>
                  )
          ...
          <div className="border-t-[6px] border-slate-100 p-4">
            ...
            {coloresPaleta.length ? (
              <>
                <div className="mt-3 flex h-12 overflow-hidden rounded-2xl ring-1 ring-black/5">{coloresPaleta.map((color, index) => <span key={`${color}-${index}`} className="flex-1" style={{ backgroundColor: color }} />)}</div>
                {paleta?.observaciones && <p className="mt-3 rounded-xl bg-pink-50 px-3 py-2.5 text-[10px] leading-4 text-slate-600 ring-1 ring-pink-100">{paleta.observaciones}</p>}
              </>
            ) : <p className="mt-2 text-xs text-slate-400">Todavía no hay una paleta publicada para este servicio.</p>}
          </div>
        </section>
      ) : (
        <section className="rounded-[24px] bg-white px-5 py-8 text-center ring-1 ring-black/[0.04]">
          <CalendarDays className="mx-auto h-6 w-6 text-slate-300" />
          <h2 className="mt-3 text-sm font-extrabold text-slate-700">Sin servicios próximos</h2>
          <p className="mt-1 text-xs leading-5 text-slate-400">Cuando un líder te programe, tu servicio aparecerá aquí.</p>
        </section>
      )}
      ...
      <p className="mt-6 text-center text-[10px] leading-4 text-slate-400">Las solicitudes de reemplazo guiadas por capacidad se incorporarán en el siguiente bloque del flujo.</p>
    </main>
  )
}
