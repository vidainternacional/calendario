...
import { CalendarDays, ChevronLeft, PencilLine, Plus, RotateCcw, Settings2, Trash2, UserPlus, Users } from 'lucide-react'
import { createAdminClient, createClient } from '@/lib/supabase/server'
import { asignarServidorAlabanza, quitarServidorAlabanza, crearFuncionMinisterial, actualizarFuncionMinisterial, cambiarEstadoFuncionMinisterial } from '@/app/actions/programacion-alabanza'
...
export default async function EquipoAlabanzaPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ mes?: string; evento?: string }> }) {
  ...
  return <main className="mx-auto min-h-screen max-w-2xl bg-[#f5f5f7] px-4 pb-[calc(7rem+env(safe-area-inset-bottom))] pt-[calc(env(safe-area-inset-top)+1rem)] sm:px-6 sm:pt-8">
    <header className="mb-5 flex items-center gap-3"><Link href={`/ministerios/${id}/programacion?mes=${mes}${evento ? `&evento=${evento.id}` : ''}`} className="grid h-11 w-11 place-items-center rounded-full bg-white shadow-sm ring-1 ring-black/[0.04]"><ChevronLeft className="h-5 w-5" /></Link><div><p className="text-[10px] font-bold uppercase tracking-[0.16em]" style={{ color }}>Panel del líder</p><h1 className="text-2xl font-extrabold tracking-[-0.03em] text-[#171923]">Programar equipo</h1><p className="mt-1 text-xs text-slate-500">{ministerio.nombre}</p></div></header>

    <section className="rounded-[24px] bg-white p-4 shadow-sm ring-1 ring-black/[0.04]">
      <div className="flex items-center gap-2"><Settings2 className="h-5 w-5 text-indigo-500"/><div><h2 className="text-sm font-extrabold text-[#171923]">Funciones de Alabanza</h2><p className="text-xs text-slate-500">El líder define las funciones que usa para programar al equipo.</p></div></div>
      <div className="mt-4 space-y-2">{(funciones as any[]).map((f:any)=><details key={f.id} className={`rounded-2xl p-3 ring-1 ${f.activo ? 'bg-slate-50 ring-slate-100' : 'bg-slate-50/50 ring-slate-100 opacity-70'}`}><summary className="flex cursor-pointer list-none items-center justify-between gap-3"><span className="min-w-0"><span className="block truncate text-xs font-extrabold text-slate-700">{f.nombre}</span><span className="mt-0.5 block text-[10px] font-bold uppercase tracking-wide text-slate-400">{f.categoria} · {f.activo ? 'Activa' : 'Retirada'}</span></span><PencilLine className="h-4 w-4 shrink-0 text-slate-400"/></summary><form action={actualizarFuncionMinisterial.bind(null,id)} className="mt-3 grid gap-2 border-t border-slate-200/70 pt-3"><input type="hidden" name="funcion_id" value={f.id}/><input name="nombre" defaultValue={f.nombre} required className="h-10 rounded-xl bg-white px-3 text-xs font-semibold ring-1 ring-slate-100"/><input name="categoria" defaultValue={f.categoria || 'Servicio'} placeholder="Categoría" className="h-10 rounded-xl bg-white px-3 text-xs ring-1 ring-slate-100"/><button className="h-10 rounded-xl bg-indigo-600 text-xs font-bold text-white">Guardar cambios</button></form><form action={cambiarEstadoFuncionMinisterial.bind(null,id)} className="mt-2"><input type="hidden" name="funcion_id" value={f.id}/><input type="hidden" name="activo" value={f.activo ? 'false' : 'true'}/><button className={`flex h-10 w-full items-center justify-center gap-2 rounded-xl text-xs font-bold ${f.activo ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-700'}`}>{f.activo ? <Trash2 className="h-3.5 w-3.5"/> : <RotateCcw className="h-3.5 w-3.5"/>}{f.activo ? 'Retirar función' : 'Reactivar función'}</button></form></details>)}</div>
      <form action={crearFuncionMinisterial.bind(null,id)} className="mt-4 grid gap-2 border-t border-slate-100 pt-4"><p className="flex items-center gap-2 text-xs font-extrabold text-slate-700"><Plus className="h-4 w-4 text-indigo-500"/>Nueva función</p><input name="nombre" required placeholder="Ej.: Guitarra principal" className="h-11 rounded-xl bg-slate-50 px-3 text-xs ring-1 ring-slate-100"/><input name="categoria" defaultValue="Instrumentos" placeholder="Categoría" className="h-11 rounded-xl bg-slate-50 px-3 text-xs ring-1 ring-slate-100"/><button className="h-11 rounded-xl bg-indigo-600 text-xs font-bold text-white">Agregar función</button></form>
    </section>

    <section className="mt-5 rounded-[24px] bg-white p-4 shadow-sm ring-1 ring-black/[0.04]"><div className="flex items-center gap-2"><CalendarDays className="h-5 w-5 text-indigo-500"/><div><h2 className="text-sm font-extrabold text-[#171923]">Servicio</h2><p className="text-xs text-slate-500">Elige una fecha real del Calendario.</p></div></div><form method="get" className="mt-4"><input type="month" name="mes" defaultValue={mes} className="h-11 w-full rounded-xl bg-slate-50 px-3 text-sm font-semibold text-slate-700 ring-1 ring-slate-100"/><button className="mt-2 h-10 w-full rounded-xl bg-slate-100 text-xs font-bold text-slate-600">Ver mes</button></form><div className="mt-4 space-y-2">{eventos.length === 0 ? <p className="rounded-xl bg-slate-50 p-4 text-center text-xs text-slate-400">Sin servicios este mes.</p> : eventos.map((e:any)=><Link key={e.id} href={`/ministerios/${id}/programacion/equipo?mes=${mes}&evento=${e.id}`} className={`block rounded-2xl p-3 ring-1 ${evento?.id===e.id?'bg-indigo-50 ring-indigo-200':'bg-slate-50 ring-slate-100'}`}><p className="text-sm font-extrabold text-slate-800">{e.titulo}</p><p className="mt-1 text-xs text-slate-500">{fechaSV(e.fecha_inicio)}{e.ubicacion?` · ${e.ubicacion}`:''}</p></Link>)}</div></section>

    {evento && <section className="mt-5 rounded-[24px] bg-white p-4 shadow-sm ring-1 ring-black/[0.04]"><div className="flex items-center gap-2"><Users className="h-5 w-5 text-indigo-500"/><div><h2 className="text-sm font-extrabold text-[#171923]">Integrantes asignados</h2><p className="text-xs text-slate-500">{fechaSV(evento.fecha_inicio)}</p></div></div><div className="mt-4 space-y-2">{asignaciones.length===0?<p className="rounded-xl bg-slate-50 p-3 text-xs text-slate-500">Todavía no hay integrantes asignados.</p>:asignaciones.map((a:any)=><div key={a.id} className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3"><div className="grid h-10 w-10 place-items-center overflow-hidden rounded-full bg-indigo-100 font-extrabold text-indigo-600">{a.persona?.avatar_url?<img src={a.persona.avatar_url} alt="" className="h-full w-full object-cover"/>:(a.persona?.nombre_completo||'U').charAt(0)}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-800">{a.persona?.nombre_completo||'Servidor'}</p><p className="truncate text-xs text-slate-500">{a.capacidad?.nombre||'Función anterior'} · {a.estado}</p></div><form action={quitarServidorAlabanza.bind(null,id,evento.id)}><input type="hidden" name="asignacion_id" value={a.id}/><button className="grid h-10 w-10 place-items-center rounded-full bg-rose-50 text-rose-500" aria-label="Quitar del servicio"><Trash2 className="h-4 w-4"/></button></form></div>)}</div>
      <div className="mt-5 border-t border-slate-100 pt-4"><p className="flex items-center gap-2 text-xs font-extrabold text-slate-700"><UserPlus className="h-4 w-4 text-indigo-500"/>Asignar integrante por función</p><p className="mt-1 text-[11px] leading-5 text-slate-500">Solo aparecen personas que tengan esa función oficial en su ficha.</p><div className="mt-3 space-y-3">{funcionesActivas.map((cap:any)=>{const candidatos=candidatosPorCapacidad.get(cap.id)||[];return <form key={cap.id} action={asignarServidorAlabanza.bind(null,id,evento.id)} className="rounded-2xl border border-slate-100 p-3"><input type="hidden" name="capacidad_id" value={cap.id}/><p className="text-xs font-bold text-slate-700">{cap.nombre}</p>{candidatos.length?<div className="mt-2 flex gap-2"><select name="profile_id" className="h-11 min-w-0 flex-1 rounded-xl bg-slate-50 px-3 text-xs font-semibold">{candidatos.map((x:any)=><option key={x.id} value={x.id}>{x.nombre_completo}</option>)}</select><button className="h-11 rounded-xl bg-indigo-600 px-4 text-xs font-bold text-white">Asignar</button></div>:<p className="mt-2 text-[11px] text-slate-400">Sin candidatos compatibles.</p>}</form>})}</div></div>
    </section>}
  </main>
}
