...
export default async function BosquejoDetallePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: bosquejo } = await (supabase as any)
    ...
    .select('id, titulo, tema, pasaje_base, proposito, introduccion, puntos, conclusion, notas_privadas, estado, fecha_predicacion')
    .eq('id', id)
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-7xl bg-[#f4f5f9] px-4 pb-[calc(8rem+env(safe-area-inset-bottom))] pt-[calc(1.5rem+env(safe-area-inset-top))] sm:px-6 sm:pt-8 lg:px-8">
      ...
      <header className="mb-6 mt-3 max-w-3xl lg:mb-8">
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-indigo-600">Editor pastoral</p>
        <h1 className="mt-2 text-2xl font-bold text-slate-900 sm:text-3xl">Preparar bosquejo</h1>
        <p className="mt-1.5 text-sm leading-6 text-slate-500 sm:text-base">Prepara, revisa y presenta el mensaje desde móvil, tablet o computadora.</p>
      </header>
