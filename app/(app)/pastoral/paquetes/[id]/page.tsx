...
export default async function PaquetePastoralDetallePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: paquete } = await (supabase as any)
    ...
    .select('id, titulo, descripcion_publica, instrucciones, bosquejo_id, coleccion_id, recurso_ids, estado')
    .eq('id', id)
  ...
  const [{ data: bosquejos }, { data: colecciones }, { data: biblioteca }] = await Promise.all([
    ...
    (supabase as any).from('pastoral_biblioteca').select('id, titulo, descripcion, categoria, tipo, url, storage_path').eq('profile_id', user.id).order('updated_at', { ascending: false }),
  ])
  ...
  const recursosSeleccionados = await Promise.all((biblioteca ?? []).filter((item: any) => idsSeleccionados.has(item.id)).map(async (item: any) => {
    ...
    if (item.tipo === 'archivo' && item.storage_path) {
      ...
      accesoUrl = data?.signedUrl ?? null
    }
    return { id: item.id, titulo: item.titulo, descripcion: item.descripcion ?? '', categoria: item.categoria, tipo: item.tipo, acceso_url: accesoUrl }
  }))
  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl bg-[#f4f5f9] px-4 pb-[calc(8rem+env(safe-area-inset-bottom))] pt-[calc(1.5rem+env(safe-area-inset-top))] sm:px-6 sm:pt-8 lg:px-8">
      <header className="mb-6 print:hidden">
        ...
        <p className="mt-1.5 max-w-3xl text-sm leading-6 text-slate-500">Edita las relaciones y revisa la guía final que recibirá la congregación.</p>
      </header>
      ...
      <PaqueteDetalleClient
        ...
        biblioteca={(biblioteca ?? []).map((item: any) => ({ id: item.id, titulo: item.titulo, categoria: item.categoria, tipo: item.tipo }))}
      />
    </main>
  )
}
